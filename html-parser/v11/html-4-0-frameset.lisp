...
;; all rights reserved.
;;
;; $Id: //depot/cl-http/html-parser-11/html-4-0-frameset.lisp#1 $
;;
...
(define-html-entity #e"%Length"
  cdata)				; nn for pixels or #e"nn%" for percentage length
...
(define-html-entity #e"%LIStyle"
  cdata)				; constrained to: "(#e"%ULStyle";|#e"%OLStyle";)
...
;;<![ #e"%HTML.Frameset"; [
;;<!ENTITY #e"%" noframes.content "(BODY) -(NOFRAMES)">
;;]]>
...
(define-html-element link		; a media-independent link
    :end-optional t
    :attributes (#e"%attrs"		; %coreattrs, %i18n, #e"%events" 
		   (charset #e"%Charset" :IMPLIED) ; char encoding of linked resource
		   (href #e"%URI" :IMPLIED) ; URI for linked resource
		   (hreflang #e"%LanguageCode" :IMPLIED) ; language code
		   (type #e"%ContentType" :IMPLIED) ; advisory content type
...
(define-html-element frameset		; window subdivision
    ...
    :attributes (#e"%coreattrs"		; id, class, style, title
		   (rows #e"%MultiLengths" :implied) ; list of lengths, default: #e"100%" (1 row)
		   (cols #e"%MultiLengths" :implied) ; list of lengths, default: #e"100%" (1 col)
		   (onload #e"%Script" :implied) ; all the frames have been loaded
		   (onunload #e"%Script" :implied) ; all the frames have been removed
		   ))
...
(define-html-element frame		; subwindow
    ...
    :attributes (#e"%coreattrs"		; id, class, style, title
		   ...
		   (noresize (:or noresize) :implied) ; allow users to resize frames?
