...
;; all rights reserved.
;;
;; $Id: //depot/cl-http/html-parser-11/defs.lisp#2 $
;;
...
(when (or *load-pathname* *compile-file-pathname*)
  (let* ((load-directory
	 ...
	 (translation (merge-pathnames #p"**/*.*" load-directory)))
    (setf (logical-pathname-translations "html-parser")
      `(("**;*.*" ,translation)))))

;;; Constants & Variables

(defvar *html-dtd-list*
...
(defun modify-dtd-list (id file)
  (let ((dtd-rec (assoc id *html-dtd-list* :test #'string=)))
    (if dtd-rec
	(if file
	    (setf (cadr dtd-rec) file)
	  (setq *html-dtd-list* (delete dtd-rec *html-dtd-list*)))
      (push (list id file) *html-dtd-list*))))
...
(eval-when (:load-toplevel :compile-toplevel)
  ...
  (defmethod make-load-form ((token html-entity-token))
    `(intern-entity-token ,(token-name token)))
  
  #-(or :ansi-cl :draft-ansi-cl-2)
  (defmethod make-load-form ((token html-name-token))
    `(intern-name-token ,(token-name token)))
  ...
  (defun make-html-name-token (string &optional start end)
    (make-instance 'html-name-token
      :name (string-upcase string
			   :start (or start 0) :end (or end (length string)))))
  
  (defun make-html-entity-token (string &optional start end)
    (make-instance 'html-entity-token
      :name (subseq string (or start 0) (or end (length string)))))
  
  ;; Tokenizer function: intern-name-token
  ;; Tokenizer for storing case insensitive HTML name strings
  #-CL-HTTP
  (define-tokenizer name
      :test #'equalp
      :constructor #'make-html-name-token)

  #+CL-HTTP
  (define-tokenizer name
      :test 'char-equal
      :tokenizer 'make-html-name-token
      :documentation "Manages case insensitive HTML name strings")
  
  ;; Tokenizer function: intern-entity-token
  ;; Tokenizer for storing case sensitive HTML entity strings
  #-CL-HTTP
  (define-tokenizer entity
      :test #'equal
      :constructor #'make-html-entity-token)

  #+CL-HTTP
  (define-tokenizer entity
      :test 'char=
      :tokenizer 'make-html-entity-token
      :documentation "Manages case sensitive HTML entity strings")
  
  #+cl-http
  (defun intern-name-token (string)
    (tokenize-name string))
  
  #+cl-http
  (defun intern-entity-token (string)
    (tokenize-entity string))
...
(defstruct (parser (:print-function print-parser))
  ...
  (read-position 0)
  (save-fragments nil)
  (make-tag-instance-fn #'parser-make-tag-instance)
  (open-tag-fn nil)
  (close-tag-fn nil)
  (cdata-fn nil)
  (pcdata-fn nil))
...
(defmethod contains ((name string))
  (contains (intern-name-token name)))
...
(defmethod inclusions ((name string))
  (inclusions (intern-name-token name)))
...
(defmethod exclusions ((name string))
  (exclusions (intern-name-token name)))
...
(defmethod containers ((name string))
  (containers (intern-name-token name)))
...
(defmethod default-container ((name string))
  (default-container (intern-name-token name)))
...
(defmethod instances ((name string))
  (instances (intern-name-token name)))
...
(defmethod attributes ((name string))
  (attributes (intern-name-token name)))
...
(defmethod start-optional ((name string))
  (start-optional (intern-name-token name)))
...
(defmethod end-optional ((name string))
  (end-optional (intern-name-token name)))
...
;;; Tag and entity accessors and predicates

(defmethod tag-definition ((tag-instance abstract-tag-instance))
  (instance-of tag-instance))

(defmethod tag-definition ((name string))
  (get-value (intern-name-token name) :tag-definition))

(defmethod tag-definition ((name html-name-token))
  (get-value name :tag-definition))

(defmethod tag-definition ((name symbol))
  (get-value (intern-name-token (symbol-name name)) :tag-definition))

(defun parameter-entity-token-p (token)
  (and (typep token 'html-entity-token)
       (char= (char (token-name token) 0) #\%)))

(defmethod entity-definition ((name string))
  (get-value (intern-entity-token name) :entity-definition))

(defmethod entity-definition ((name html-entity-token))
  (get-value name :entity-definition))
