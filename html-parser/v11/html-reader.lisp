...
;; all rights reserved.
;;
;; $Id: //depot/cl-http/html-parser-11/html-reader.lisp#1 $
;;
...
(defun process-name-token (string &optional (start 0) (end (length string)))
  (intern-name-token (subseq string start end)))

(defun process-entity-token (string &optional (start 0) (end (length string)))
  (intern-entity-token (subseq string start end)))
...
(defun html-whitespace-p (char)
  ...
  (case char
    ((#\space #\tab #\linefeed #\return #-Genera #\newline) t)))
...
(defun next-html-char (parser &optional (update-p t))
  (declare (optimize (speed 3) (space 1) (debug 0)))
...
(defun next-char-space-p (parser)
  (declare (optimize (speed 3) (space 1) (debug 0)))
  (let ((input (parser-input parser))
        (pos (parser-read-position parser))
        (input-length (length (parser-input parser))))
    (when (< pos input-length)
      (and (html-whitespace-p (char input pos))
	   (not (or (= pos 0) (html-whitespace-p (char input (1- pos)))))))))
    ...
    (when (< pos input-length)
	  ...
	  until (when (and (eql (char input npos) #\<)
		  ...
		  (eq (name (parser-cdata-element parser))
		      (parse-html-name parser #'process-name-token)))
...
(defun initialize-parse-dispatch-functions ()
  (setf (get-value #t"CDATA" :quoted-parser) #'parse-html-literal
	...
	#'(lambda (parser)
	    (parse-html-literal parser #'process-name-token))
...
(defun tag-value-attribute (value tag-defn)
  ;; Locates the attribute definition for the value, or uses #t"unknown"
  ;; Perhaps it should be +the-unknown-attribute+?
  ...
  (if attr-defn
	...
	(html-name-token
	 (funcall (get-value (allowed-values attr-defn) :quoted-parser)
		  parser))
	(list (parse-html-literal parser #'process-name-token)))
  ...
  (if attr-defn
	...
	(html-name-token
         (funcall (get-value (allowed-values attr-defn) :unquoted-parser)
		  parser))
	(list (parse-html-name parser #'process-name-token)))
...
(defun parse-html-attr-name-or-val (parser tag-defn rattr-list)
  (let ((name-or-val (parse-html-name parser #'process-name-token)))
...
(defun make-tag-instance (tag-name tag-defn attr-vals parser)
  (when tag-name
    (if tag-defn
	(funcall (parser-make-tag-instance-fn parser)
		 :instance-of tag-defn :attr-values attr-vals)
      (funcall (parser-make-tag-instance-fn parser)
	       :instance-of (tag-definition #t"UNKNOWN")
	       :name tag-name :attr-values attr-vals))))

(defun parse-html-open-tag (parser)
  (declare (values tk-type tag-instance))
  (let* ((tag-name (parse-html-name parser #'process-name-token))
...
(defun parse-html-entity (parser)
  (declare (values tk-type tk-value))
  (let ((entity-name (parse-html-name parser #'process-entity-token)))
    (when (entity-definition entity-name)
