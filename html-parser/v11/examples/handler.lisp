;;; -*- Mode: Lisp; Syntax: Ansi-Common-Lisp; Package: HTML-PARSER; Base: 10 -*-

;; (c) Copyright 1996-98,2000 Sunil Mishra <smishra@everest.com>
;;     All Rights Reserved

;; $Id: //depot/cl-http/html-parser-11/examples/handler.lisp#1 $
...
(defmacro with-tag-handler ((tags handler-fn &key with-attributes 
  ...
  (flet ((to-string (name)
         ...
         (test-tag (tag with-attrs without-attrs)
           (let ((tag-defn (tag-definition tag)))
             (assert (or (member tag *character-data-classes*)
                         tag-defn)
                     ()
	       "~A does not name a tag name or character data class" tag)
             (assert (every #'(lambda (attr)
				(find attr (attributes tag-defn) :key #'name))
                            with-attrs)
                     ()
    ...
    (let* ((tag-strings (cond ((eql tags t) t)
           ...
           (tag-tokens (if (eql tags t)
                           t
                           (mapcar #'intern-name-token tag-strings)))
           (with-attr-strings (mapcar #'to-string with-attributes))
           (with-attr-tokens (mapcar #'intern-name-token with-attr-strings))
           (without-attr-strings (mapcar #'to-string without-attributes))
           (without-attr-tokens (mapcar #'intern-name-token 
					without-attr-strings)))
      (cond ((eql tag-strings t)
	       ...
	       "No other handlers may be specified with a universal specification"))
            (t (mapc #'(lambda (tag-name)
                         (test-tag tag-name with-attr-tokens
				   without-attr-tokens))
                     tag-tokens)
        ...
        (let ((handler-rec (find (ecase tag-type
          ...
          (and handler-rec
               (or (and (eql tag-type :pcdata)
                   ...
                   (and (subsetp (handler-with-attributes handler-rec)
				 (attr-values tag-data)
                                 :test #'match-attr-val-pair)
                        (null (intersection
			       (handler-without-attributes handler-rec)
			       (attr-values tag-data)
			       :test #'match-attr-val-pair))
                        (funcall (handler-test handler-rec)
				 tag-type tag-data)))))
