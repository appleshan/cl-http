...
;; portions copyright (c) 1996-1998 John C. Mallery <jcma@ai.mit.edu>,
;; all Rights Reserved

;; $Id: //depot/cl-http/html-parser-11/examples/parser-show-link-map.lisp#1 $
...
(defun make-absolute-url (url-string)
  ...
  (let ((url-buffer
	 (make-array (length url-string) :fill-pointer 0
	...
	(cond ((or http-p relative-p)
		  ...
		  (let ((url:*escape-search-urls* nil))
		    (url:intern-url (merge-walker-url url-buffer) 
...
(eval-when (:load-toplevel :compile-toplevel)
  (defun open-resource-with-content-test-def (stream-var headers-var activity
					      url major-mime-type
					      minor-mime-type body)
    (let ((headers-var-internal (gensym))
      ...
      `(multiple-value-bind (,headers-var-internal ,status-code-var)
	   (handler-case-if (not *debug-walker*)
	     ...
	     (error (err)
	       (record-url-note ,activity ,url :error-getting-headers
				(report-string err))
	       (values nil 500)))
	 (case ,status-code-var
	   (200
	    (destructuring-bind (&optional ,major-var ,minor-var
				 &rest ,plist-var)
		(get-header :content-type ,headers-var-internal)
	      (cond ((and ,(if (atom major-mime-type)
		     ...
		     (multiple-value-bind (,stream-var ,headers-var
					   ,status-code-var)
    ...
    (let ((status-code-var (gensym)))
      `(multiple-value-bind (,stream-var ,headers-var ,status-code-var)
	   (handler-case-if (not *debug-walker*)
			    (get-resource-content ,activity ,url)
	     (error (err)
	       (record-url-note ,activity ,url 
				:error-getting-headers (report-string err))
	       (values nil 500)))
	 (when (<= 200 ,status-code-var 300)
	   ,@body))))
  )
...
(defmacro with-resource-stream ((stream-var headers-var activity url
				 &key (major-mime-type t) (minor-mime-type t)) 
				&body body)
...
(html-parser:define-html-parser-context href-filter-context ()
  ...
  (:on-close-tag ((a img href)		; Figure out what the URL is
		  (let ((href-val (html-parser:attr-val
				   (if (eq (html-parser:name it) #t"img")
				       "src"
				     "href")
				   it)))
		 ...
		 (a (setq inside-anchor-p nil)
			...
			(or (html-parser:make-pcdata-string
			     (html-parser:parts it))
			    (html-parser:make-pcdata-string
			     (html-parser:parts it) t)
			    (url:name-string url)))))
		 (img (when url
			(setf (get-value url :description)
...
;;; GENERATE-INFERIORS-VIA-PARSING
...
(defmethod generate-inferiors-via-parsing ((url url:http-url) (activity activity))
  (with-resource-stream (document *headers* activity url
				  :major-mime-type :text
				  :minor-mime-type :html)
...
(define-action-type
  ...
  (multiple-value-bind (headers status-code)
	...
	((and status-code (not (<= 200 status-code 299)))
	 ...
	 (format stream "~&Status: ~D ~A"
		 status-code (get-string-for-status-code status-code t))) 
	(last-modified
	  (html:break-line :stream stream)
	  #+genera
	  (let ((date (get-header :date headers)))
	    (if date
		(format stream "~&Resource Age: ~\\time-interval\\"
			(- date last-modified))
		(http::write-header :last-modified last-modified stream)))
	  #-genera
	  (http::write-header :last-modified last-modified stream))))))

(defmethod show-web-link-map ((url http-url)
			      &key (operator http:*server-mail-address*)
				   headers-p (depth 3) subdirectories-p hosts
				   minimize-dns-p respect-no-robots-p
				   constraints (stream *standard-output*))
  (let ((title "Web Link Structure")
	...
	(html:with-section-heading (title :stream stream)
	  (write-show-web-link-map-legend
	   url operator depth subdirectories-p minimize-dns-p
	  ...
	  (html:with-paragraph (:stream stream)
		...
		("Show-Web-Link-Map"
		 (:operator operator
		  ...
		  :url-host-name-resolution (if minimize-dns-p
						:never
					      :preferred)
