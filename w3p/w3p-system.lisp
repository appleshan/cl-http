...
(defmacro get-class-precedence-list (class)
  `(#+CMU pcl:class-precedence-list
          ...
          #+CLOZURE-COMMON-LISP ccl:class-precedence-list
    #-(or MCL CMU sbcl CLOZURE-COMMON-LISP) clos:class-precedence-list
    ,class))
...
(defun get-superiors-from-inherit-from (inherit-from)
  ...
  (flet ((extract-name (backquote)
           #+(or MCL Genera Lispworks Lucid CMU scl sbcl CLOZURE-COMMON-LISP)
    ...
    (cond ((atom inherit-from) (list inherit-from))
          ((eql 'and (car inherit-from)) (mapcar #'extract-name (cdr inherit-from)))
