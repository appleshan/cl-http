    ...
    (let ((output-pathname
      ...
      (with-open-file (stream output-pathname
        ...
        (let ((*package* (find-package "BNF-PARSER"))
          ...
          (print `(in-package ,(package-name "BNF-PARSER")) stream)
          (map nil #'(lambda (definition)
                       (when (and (consp definition) (eq (first definition) 'defun))
                         (print (list 'declaim `(ftype (function (t) t) ,(second definition)))
                                stream)))
               form)
      ...
      #|(when (or load execute)
        (load (if (and compile fasl-pathname)
                fasl-pathname
                (namestring
                 (merge-pathnames (make-pathname :type :unspecific) output-pathname)))))|#
                     ...
                     (handler-bind
                       ((continuable-parse-error
                        ...
                        (terminal-parse-error
                         #'(lambda (condition)
                             ;; if terminating, then terminate the parse. otherwise pass it on
                             (case (condition-action condition)
                               (:abort (warn "terminating parse on condition: ~a: Context ~s." condition *atn-input)
                                       (return-from ,*parser-name (values nil condition)))
                               (:continue (warn "continuing parse after condition: ~a: Context ~s." condition *atn-input)
                                          (continue condition)))))
                        (error #'(lambda (condition)
                                   ;; unexpected errors always trave up.
                                   (warn "parse raised condition: ~a: Context ~s." condition
                                         (parse-state (atn-input)))
                                   nil)))
...
(defMethod make-lisp-form ((node atn))
  ...
  (let* ((net-name (atn-name node))
    ...
    (setf (get net-name :internal-time) 0)
    ...
    (setf form
          `(,net-name (index &aux
                      ...
                      (declare (optimize (speed ,(if *atn-trace* 1 3)) (safety ,(if *atn-trace* 3 0))))
                            ...
                            (%atn-block ,net-name
                              (labels ((succeed (index result)
                                ...
                                ,@(unless *atn-ambiguous
                                  `((declare (inline succeed fail #-LispWorks,@(mapcar #'first functions)))))
                              ...
                              (labels ((succeed (index result)
                                       ...
                                       (fail (index)
                                         ...
                                         ,(if *atn-ambiguous
                                            'index
                                            `(return-from ,net-name (values nil index nil))))
                                       ,@(progn (setf functions (sort (mapcar #'make-lisp-form (atn-nodes node))
                                                                      #'string-lessp :key #'first))
                                           (when *atn-ambiguous functions)))
                                ;;,@(unless *atn-ambiguous
                                ;; `((declare (inline succeed fail #-LispWorks,@(mapcar #'first functions)))))
                                ;; to start invoke the function for the initial arc
                                ,@(atn-initial-actions node)
...
(defMethod make-lisp-form ((node atn-node))
  ...
  (let ((node-name (atn-name node))
    ...
    `(,node-name ,@(when *atn-ambiguous `((index ,@(when node-bindings `(&aux ,@node-bindings)))))
       ...
       (cond ((,cat-predicate |item|)
              ...
              ,@(subst `(if (eq *atn-reduce* t)
                       ...
                       (case (atn-term-cardinality atn register-name)
                         ...
                         ((+ *) `((setf ,register-name (cons :result ,register-name))))))
              ,@(atn-succeed-actions node)
              ,(if *atn-ambiguous `(,end (1+ index)) `(progn (incf index) (go ',end))))
             (t
              (setf *atn-term* ',register-name)
              ,@(atn-fail-actions node)
              ,(if fail `(,fail index) nil))))))
                       ...
                       (if *atn-trace*
                         (if constructor-specializer
                           `(atn-reduce-structure-with-context (function ,constructor)
                                                               ,constructor-specializer ,@term-names)
                           `(atn-reduce-structure (function ,constructor) ,constructor-specializer ,@term-names))
