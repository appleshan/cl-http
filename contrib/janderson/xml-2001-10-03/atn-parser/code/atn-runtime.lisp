...
(defMacro %atn-block (name &rest body)
  (if *atn-trace*
    `(progn
       ...
       #+ccl (when (and (ccl:option-key-p) (ccl:command-key-p)) (break))
       ,@body)
    (if (cdr body)
      `(progn ,@body)
      (first body))))
...
(defMethod input.peek-item ((input atn-sequence-input) position)
  "the method for sequences is the same input.item.
   where the input is generated or read, this will differ."
...
(defParameter *terminal-parse-error-action* nil
  "specifies how parser reacts to non-continuable errors.
   the value <code>NIL</code>, the default value, allows the signalled error to pass up to the caller.
   the value <code>:CONTINUE</code> permits the parse to continue, but uses <code>NIL</code> as the effective
   parse for the respective term.
   the value <code>:ABORT</code> terminates the parse with the value <code>NIL</code>.")
...
(define-condition parse-error (error)
  ...
  (:report (lambda (condition stream)
             (let ((string (condition-format-string condition)))
               #-Genera(call-next-method)
               (fresh-line stream)
               (format stream "parse error~@[ with-state ~s~]."
...
(defMethod initialize-instance :after ((instance parse-error) &key
                                &aux (input (atn-input)))
  (with-slots (state) instance
    (setf state (parse-state input))))

(defMethod condition-parse-state ((instance parse-error))
  (with-slots (state) instance state))
         

(define-condition terminal-parse-error (parse-error)
  ((action :initform *terminal-parse-error-action* :reader condition-action)))

(eval-when (:load-toplevel :compile-toplevel :execute)
  (define-condition continuable-parse-error (parse-error)
     ((continue-format-string :initform "attempt to continue to parse."
                              :reader condition-continue-format-string)
      (continue-format-arguments :initform nil
                                 :reader condition-continue-format-arguments)))
)

(define-condition parse-eof-error (terminal-parse-error end-of-file) ())
...
(defun atn-reduce-structure (constructor &rest structure)
  ...
  (handler-bind ((continuable-parse-error
                  #'(lambda (condition)
