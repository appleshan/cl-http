...
(defMethod uri ((node package)) (or (first (package-nicknames node)) (package-name node)))
...
(defGeneric find-element-by-id (node id)
  ...
  (:method ((node elem-node) (id string))
           (or (some #'(lambda (attribute)
                         (let ((value (value attribute)))
                           (when (and (typep value 'id-value)
                                      (string= id (value-string value)))
                             node)))
                     (attributes node))
               (some #'(lambda (child)
                         (find-element-by-id child id))
                     (children node))))
  (:method ((node doc-node) (id string))
           (with-slots (id-cache root) node
             (if id-cache
               (gethash id id-cache)
...
(defGeneric walk-node (node predicate function)
  ...
  (:method ((node doc-node) predicate function)
           (when (funcall function node)
             (with-slots (uri children parameter-entities general-entities) node
               (walk-node uri predicate function)
               (dolist (node children) (walk-node node predicate function))
               (dolist (node general-entities) (walk-node node predicate function))
               (dolist (node parameter-entities) (walk-node node predicate function)))))
