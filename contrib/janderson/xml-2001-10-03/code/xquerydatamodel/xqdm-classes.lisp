...
(defClass ncnamed-node (named-node) () (:metaclass abstract-class))
...
(defClass doctype-child-node (document-scoped-node)
  ()
  (:metaclass abstract-class)
...
(defClass attr-node (elem-property-node abstract-attr-node)
  ()
  (:metaclass abstract-class))
...
(defClass abstract-ns-node () () (:metaclass abstract-class))

(defClass ns-node (elem-property-node abstract-ns-node)
...
(defClass def-elem-type (unamed-node abstract-def-type abstract-top-level-def-node
...
(defClass def-attr (abstract-top-level-def-node unamed-node qname-context-delegate)
...
(defClass def-entity (ncnamed-node abstract-top-level-def-node abstract-value-node)
  ...
  (:metaclass abstract-class))
(defClass def-general-entity (def-entity) () (:metaclass abstract-class))
(defClass def-parameter-entity (def-entity) () (:metaclass abstract-class))
...
(defClass ref-entity (ref-node named-node attr-child-node) () (:metaclass abstract-class))
...
(defClass |content-model| (bnfp::bnf-phrase-definition) ())
(defClass |\|-content| (bnfp::bnf-alternative) () )
(defClass |?-content| (bnfp::bnf-opt-expr) () )
(defClass |*-content| (bnfp::bnf-rep-expr) () )
(defClass |bounded-content| (bnfp::bnf-qexpr)
  ((min :initarg :min :initform nil)
   (max :initarg :max :initform nil)) )
(defClass |MIXED-content| (|*-content|) () )
(defClass |+-content| (bnfp::bnf-plus-expr) () )
(defClass |content| (bnfp::bnf-one-expr) () )
(defClass |,-content| (bnfp::bnf-group) () )
(defClass |content-name| (bnfp::bnf-phrase-symbol) () )
(defClass |type-name| (bnfp::bnf-nonterminal-symbol) () )
  ...
  (when (and (validate? *document*) is-mixed)
    (mapl #'(lambda (rest &aux (name (first rest)))
              (when (find name (rest rest)
                  ...
                  (xml-error-|VC: No Duplicate Types| :context model :name (first rest))))
          names))
  names)
...
(defClass qname-context ()
  ((name :initarg :name :reader name)
   (def :initform nil :initarg :def :reader def)
          ...
          (children
           (setf value (mapcar #'(lambda (name &aux entity)
                                   (unless (is-ncname name)
                                     (xml-error-|VC: ID| :name (name node) :value value
                                   ...
                                   (when document
                                     (when (setf entity (find-def-general-entity name document))
                                       (unless (and (is-def-external-general-entity entity)
                                                    (notation entity))
                                         (xml-error-|VC: Entity Name| :name name))))
                                   name)
    ...
    (cond (value)
          (t
           (unless (is-ncname (setf value
             ...
             (xml-error-|VC: Entity Name| :context (name node) :name value
                        :detail "not an NCName"))
    ...
    (cond (value)
          (children
           (unless (is-namechardata (setf value
                                            (normalize-value node (value-string children))))
             (xml-error-|VC: Name Token| :name (name node) :value value
                        :detail "not a Name"))
           (setf value (intern-name value *null-namespace*))))))
          ...
          (children
           ...
           (when def
             (unless (find value (enumeration def))
               (xml-error-|VC: Notation Attributes| :name value))))
          (t
           (xml-error-|VC: Name Token| :name (name node) :value value
                               :detail "not an Name")))))
          ...
          (children
           ...
           (when def
             (unless (find value (enumeration def) :test #'string=)
               (xml-error-|VC: Enumeration| :name value)))))))
  ...
  (with-slots (document) instance
    (when (setf document (document parent))
      (when (gethash id (ids document))
        (xml-error-|VC: ID| :name (name instance) :value id :detail "not unique"))
...
(defMethod clone-node
  ...
  (when initargs (apply #'shared-initialize instance nil initargs))
  ...
  (with-slots (node-class name) def
    (or node-class
        (cond ((and *specialize-elem-node*
                    (setf node-class (find-class name nil))
                    (subtypep name 'abstract-elem-node))
               node-class)
              (t
...
(defMethod (setf children) :before (children (instance elem-node))
  (with-slots (def) instance
    ...
    (dolist (n children)
      (etypecase n
        ...
        (string )))
    (when (validate? instance)
      (unless (validate-content def children)
        (xml-error-|VC: Element Content| :model (when def (children def))
                   :content children)))))
  ...
  (with-slots (prototype children) instance
    ...
    (mapl #'(lambda (values)
              (when (find (first values) (rest values) :test #'bnfp::name=)
                (xml-error-|VC: No Duplicate Tokens| :context instance :name (first values))))
          children)))
...
(defMethod print-object ((instance qname-value) stream)
  (with-slots (namespace name) instance
    (format stream "{~@[~a~]}~a" (when namespace (package-name namespace)) name)))
...
(defMethod print-object ((instance def-internal-entity) stream
                         &aux children trimmed)
  (print-unreadable-object (instance stream :type t :identity t)
    (prin1 (name instance) stream)
    (setf children (children instance))
    (write-string " '" stream)
    (dotimes (i (length children))
      (write-char (code-char (pop children)) stream)
      (when (> i 16) (setf trimmed t) (return)))
    (if trimmed
      (write-string "..." stream)
      (write-char #\' stream))))
  ...
  (with-slots (name children) node
    (print-unreadable-object (node stream :type t :identity t)
      ...
      (if (typep children '|content-model|)
        ...
        (format stream "~s ~a" name children)))))
...
(defGeneric print-qname (stream symbol &rest args)
  (:method ((stream stream) (symbol symbol) &rest args)
           ...
           (let ((prefix (prefix symbol))
             ...
             (etypecase namespace
               (package (setf namespace (package-name namespace)))
               (null t)
               (string t))
             (if namespace
             ...
             symbol))
  (:method ((stream stream) (name abstract-name) &rest args)
           (declare (ignore args))
           (let ((prefix (prefix name))
             ...
             (etypecase namespace
               ...
               (nameset-tokenizer (setf namespace (tokenizer-name namespace)))
               (null t)
               (string t))
             (if namespace
  ...
  (:method ((name abstract-name) (stream stream) &rest args)
           (declare (dynamic-extent args))
  ...
  (:method ((datum t) (stream stream) &rest args)
           (declare (ignore args))
           (prin1 datum stream))
  (:method ((node named-node) (stream stream) &rest args)
           (declare (dynamic-extent args))
           (apply #'print-qname stream (name node) args))
  (:method ((stream stream) (node named-node) &rest args)
           (declare (dynamic-extent args))
           (apply #'print-qname stream (name node) args))
  (:documentation
...
(defGeneric print-ns-node (stream node &rest args)
  ...
  (:method ((instance ns-node) (stream stream) &rest args)
           (declare (dynamic-extent args))
           (apply #'print-ns-nodes stream instance args)))
