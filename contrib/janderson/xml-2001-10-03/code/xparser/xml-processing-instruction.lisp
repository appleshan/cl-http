...
(defMacro defPiFunction (name args &rest body)
...
(defPiFunction |ns_make| (target arg)
...
#+namespace-algebra
(defPiFunction |ns_nickname| (name arg)
  (|ns_make| name arg))

#+namespace-algebra
(defPiFunction |ns_shadow| (name arg)
  (declare (ignore name))
  (destructuring-bind (ns &rest names)
                      (split-seq arg #'xml-space?)
    (setf ns (find-pi-namespace ns))
...
#+namespace-algebra
(defPiFunction |ns_unuse| (name arg)
  (declare (ignore name))
  (destructuring-bind (using &rest to-use)
                      (mapcar #'find-pi-namespace (split-seq arg #'xml-space?))
    (unuse-namespace to-use using)))

#+namespace-algebra
(defPiFunction |ns_use| (name arg)
  (declare (ignore name))
  (destructuring-bind (using &rest to-use)
                      (mapcar #'find-pi-namespace (split-seq arg #'xml-space?))
