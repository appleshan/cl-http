...
(defGeneric namespace-prefix (namespace)
  (:documentation "locate the bound prefix value for a given namespace. should there
                   be no value, signal a continuable error and permit the process to
                   bind a prefix on-demand.")
  (:method ((namespace (eql *null-namespace*)))
           *default-namespace-attribute-name*)
  (:method ((ns string))
           (namespace-prefix (find-namespace ns :if-does-not-exist :error)))
  (:method ((namespace t))
           (or (find-bound-namespace namespace *namespace-bindings*)
               (restart-case (xml-error |NSC: Prefix Declared| :name namespace)
                 (use-value (prefix) prefix)))))
...
(defMethod stream-reader ((entity def-internal-entity))
  (let ((pad? (and (is-def-parameter-entity entity) (not (in-literal?))))
        ...
        (position 0))
    (declare (type fixnum position) (type cons input) (optimize (speed 3) (safety 0)))
...
(defun advance-input ()
  (declare (type fixnum *input-column*)
           (ftype (function (t) t) *input-reader*)
          ...
          (t
           (let ((result (get-buffer-string *token-length*))
             ...
             (dotimes (i *token-length*)
               (setf (schar result i) (code-char (first buffer))
                     buffer (rest buffer)))
             (setf *token-length* 0
                   *token-end* nil
                   *token-fill* *token-start*)
...
(defun peek-token-string ()
  (when (> *token-length* 0)
    (let ((result (make-string *token-length*))
          (buffer *token-start*))
      (dotimes (i *token-length*)
        (setf (schar result i) (code-char (first buffer))
              buffer (rest buffer)))
