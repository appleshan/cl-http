...
(defParameter *discrepancies*
  '((p04pass1 . "PE: tag '<A.-:̀·/>' is a qualified name")
    ...
    (p66pass1 . "IR: character is outside of mcl range.")
    ...
    (ibm02v01 . "IR: characters beyond supported limit at offset 281.")
    ...
    (ibm66v01 . "IR: characters beyond supported limit.")
    ...
    (|052| . "IR: character beyond supported limit.")
    (|064| . "IR: character beyond supported limit.")
    (|074| . "VR: group/pe nesting not enforced")
    (|076| . "VR: a notation is used prior to definition.")
    (|089| . "IR: character beyond supported limit.")
...
(defun report-parse (pathname
                     ...
                     &aux (bnfp:*terminal-parse-error-action* :signal)
                     (*print-length* 3))
  (let ((result nil)
...
(let ((case-count 0)
   ...
   (map nil
       #'(lambda (location)
           (destructuring-bind (directory &rest args &key validate pass (reduce t)) location
             ...
             (map nil #'(lambda (path)
                          (when  (report-parse path :reduce reduce :validate validate
                                             :pass pass)
                            (incf conformant-count))
                          (incf case-count))
                  (directory directory))))
       '((#P"xmlconf:oasis;*pass*.xml" :pass t)
  ...
  (print (list :cases case-count :conformant conformant-count)))
...
(let ((*xml-verbose* t))
  (document-parser #p"www:Dokumente:Standards:XML:XMLConf:xmlconf:xmltest:not-wf:sa:084.xml"
                   :validate nil :trace nil))

(let ((*xml-verbose* t))
  (document-parser (first (directory #p"xmlconf:sun;valid;*.xml"))
