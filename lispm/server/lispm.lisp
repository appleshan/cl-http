...
(eval-when (load eval compile)
  (mapc #'(lambda (x)
        ...
        (list (intern "FIXNUM-MICROSECOND-TIME" :time)
	      ...
	      (intern "WITHOUT-PREEMPTION" :process)
...
(define-macro with-tcp-port-for-protocol ((protocol port) &body body)
  ...
  `(case (car (rassoc ,protocol tcp::*tcp-protocol-alist*))
     (,port ,@body)
...
At 3am the next 3am is on the next day."
  (multiple-value-bind (seconds minutes hours date month year day-of-the-week)
    ...
    (+ (if (not (minusp (- (+ seconds (* 60 minutes) (* #.(* 60 60) hours))
                           #.(* 3 60 60)		;3am
                           offset)))
...
(declaim (inline %pathname-as-directory))
...
(defmethod file-length-in-bytes ((pathname pathname) &optional new-length)
  ...
  (let* ((plist (cdr (fs:file-properties pathname)))
    ...
    (cond ((and byte-size length-in-bytes)
	   (if (= byte-size 8)
	       length-in-bytes
	       (ceiling (* length-in-bytes byte-size) 8)))
	  (t length-in-bytes))))
...
(export 'clean-up-file-versions :www-utils)
...
(defmethod url::pathname-ftp-url-string ((pathname fs:random-pathname) &optional force-directory-p)
  (flet ((directory-string (string)
	   (let* ((length (length string))
		  (last (position #\/ string :start 0 :end length :from-end t)))
	     (cond ((null last) "/")
		   ((eql (1+ last) length)
		    string)
		   (t (subseq string 0 (1+ last)))))))  
    (declare (inline directory-string))
    (let* ((host (pathname-host pathname))
	   (name (pathname-external-name-string (pathname-name pathname)))
	   (host-string (string-downcase (host-http-name host)))
	   (relative-url (if force-directory-p (directory-string name) name)))
      (url::make-ftp-url-string host-string nil relative-url nil))))
...
;;;------------------------------------------------------------------- 
;;;
;;; FAST MIME MULTIPART DECODING
;;;

(declaim (special http::*mime-multipart-block-maximum-size*)
	 (type function http::signal-mime-multipart-block-maximum-size-exceeded))

;; Doesn't handle small buffer, second wrap in the suffix 5/12/2001 -- JCMa.
...
;; Doesn't handle two buffer wrap arounds in the suffix 5/12/2001 -- JCMa.
...
;;;------------------------------------------------------------------- 
;;;
;;; STANDARDIZE-LINE-BREAKS SUPPPORT
;;;
...
(cp:define-command (com-standardize-file-line-breaks :command-table "File" :provide-output-destination-keyword t)
    ((pathname '((cl:sequence ((fs:pathname))))
	       :confirm t
	       :prompt "file"
	       :documentation "File or directory for line break standardization")
     &key
     (line-break '((cl:member :cr :crlf :lf))
		 :mentioned-default :yes
		 :default :cr
		 :documentation "Which end of line convention to use.")
     (report 'scl:boolean :prompt "Report" :mentioned-default nil :default t 
	     :documentation "Report each operation."))
   (flet ((wild-p (component)
	    (eq :wild component)))
     (declare (inline wild-p))
     (loop with stream = (when report *standard-output*)
	   for path in pathname
	   do (let ((name (pathname-name path))
		    (type (pathname-type path))
		    (version (pathname-version path)))
		(cond ((and (wild-p name) (wild-p type) (wild-p version))
		       (http::standardize-line-breaks (make-pathname :name nil :type nil :version nil :defaults path)
						      line-break stream))
		      ((or (wild-p name) (wild-p type) (wild-p version))
		       (dolist (p (directory path :directories t))
			 (http::standardize-line-breaks p line-break stream)))
		      (t (http::standardize-line-breaks path line-break stream))))))
   (values))
