...
;;; (C) Copyright 1994-2001, 2006, John C. Mallery.
;;;     All Rights Reserved.
;;;
    ...
    (case input-mode
      ((:ascii :ascii-crlf :ascii-suspended)
       (cond (new-input-buffer-p
              ...
              ;; Set up the indirect array only once when new input buffers arrive, but users don't get bounds checking on the high end.
...
(defun-in-flavor (%note-body-end chunk-transfer-encoding-output-stream-mixin) (buffer output-index)
  ...
  #+ignore(format *trace-output* "~&~'bChunk-End:~   ~'bBuffer~ ~S ~'bIndex~ ~4D ~'bLimit~ ~D "
		  buffer si:stream-output-index si:stream-output-limit)
  (when chunk-function
...
(defmethod (note-last-chunk chunk-transfer-encoding-output-stream-mixin) (&optional footers-plist)
  (flet ((ensure-chunk-headroom (amount)
    ...
    (cond ;; first case is a little weird. Why would we see it and why handle it this way? 9/19/2001 -- JCMa.
      ((and (null chunk-output) (null si:stream-output-buffer))	;there is no stream buffer and not chunking
	 ...
	 (unless (or (null si:stream-output-buffer)	;no buffer due to a preceeding force-output 9/19/2001 -- JCMa.
		     (= chunk-body-start si:stream-output-index))	;no data written to buffer yet
	   (multiple-value-setq (si:stream-output-index)
...
(defwhopper (:read-input-buffer chunk-transfer-decoding-input-stream-mixin) (&optional eof no-hang-p)
  (cond ((not chunked-input) (continue-whopper eof no-hang-p))
	((zerop input-chunk-size)		;Zero-sized chunk signals the end of chunk-encoded data
	 (if eof				;Continue to signal eof until chunked-input is cleared.
	     (signal 'end-of-chunk-transfer-decoding :stream self)
...
(defflavor tcp-modal-http-stream 
           ((local-protocol :http) ;choice of :http or :https (for SSL)
...
(defmethod (local-protocol tcp-modal-http-stream) ()
  local-protocol)

(export 'local-scheme :tcp)
...
;; Don't use si:stream-copy-until-eof because it forces outputs on the output stream. 9/19/2001 -- JCMa.
  ...
  (if delta
	...
	(with-input-mode (stream :binary)
	  (loop with remaining-bytes = delta and buf-size
		doing (multiple-value-bind (buffer offset limit)
			  (send stream :read-input-buffer)
			;; when a null buffer enountered formerly signalled
			;; "READ-INPUT-BUFFER failed to signal EOF. This should never happen."
			;; But not symetric with no delta case and we could be hitting the buffer
			;; boundary where the other end goes way. 12/17/2000 -- JCMa.
			(cond ((null buffer)
			       (return))
