...
(SCT:FILES-PATCHED-IN-THIS-PATCH-FILE 
  "HTTP:CLIENT;CONNECTION.LISP.81")


(EVAL-WHEN (:COMPILE-TOPLEVEL :LOAD-TOPLEVEL)
  (SCT:REQUIRE-PATCH-LEVEL-FOR-PATCH '(CL-HTTP 70. 126.)))


;========================
(SCT:BEGIN-PATCH-SECTION)
(SCT:PATCH-SECTION-SOURCE-FILE "HTTP:CLIENT;CONNECTION.LISP.81")
...
(defmethod note-server-http-version-on-port (host-object http-port http-version)
  (flet ((update-entry (key entry foundp)
    ...
    (unless (%string-test "HTTP/" (symbol-name http-version) 0 char-equal)
      (error "HTTP Client: Bad HTTP version ~S" http-version))
