...
;;; Patch file for HTTP-PROXY version 6.14
;;; Reason: Function HTTP:DISABLE-PROXY-SERVICE:  -
;;; Function HTTP:ENABLE-PROXY-SERVICE:  -
;;; Written by JCMa, 12/19/00 03:24:04
...
(SCT:FILES-PATCHED-IN-THIS-PATCH-FILE 
  "HTTP:PROXY;CACHE.LISP.152")


;========================
(SCT:BEGIN-PATCH-SECTION)
(SCT:PATCH-SECTION-SOURCE-FILE "HTTP:PROXY;CACHE.LISP.152")
...
PORT is either a port number or a list of port numbers."
  (flet ((enable-proxy-on-port (port)
	   ...
	   (pushnew port *proxy-service*)
	   (log-event :normal "Proxy Service Enabled on port ~D." port)))
    (etypecase port
      (atom
...
PORT is either a port number, a list of port numbers, or :ALL."
  (flet ((disable-proxy-on-port (port)
	   ...
	   (log-event :normal "Proxy Service Disabled on port ~D." port)))
    (etypecase port
      (integer
	...
	(ecase port
	  (:all
	    (cond (*proxy-service*
		   (log-event :normal "Proxy Service Disabled on port~P~{ ~D~^,~}."
			      (length *proxy-service*) *proxy-service*)
