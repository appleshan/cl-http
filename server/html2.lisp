...
;;;
;;; (C) Copyright 1994-97, 2001, 2003, 2005-2006, John C. Mallery.
;;;     All Rights Reserved.
;;;
...
(define write-string-quoting-specials (string &optional (stream *output-stream*) (start 0) end)
  ...
  (flet ((%token-spec-for-special-char (char)
    ...
    (with-fast-array-references ((vector string string))
      (loop with scan-idx
            for idx fixnum upfrom start below (or end (length vector))
            for char = (aref vector idx)
...
;;;------------------------------------------------------------------- 
;;;
;;; BASIC TOKEN WRITING PRIMITIVES
;;;
		 ...
		 (typecase reference
		   ...
		   (t (write-string (coerce-url-string reference url:*escape-search-urls*) stream)))
                 (etypecase local-reference
                   (string
                    (write-string local-reference stream))
                   (function (funcall local-reference stream))))))
...
(defconstant *rendition* '((:italic . "I")
                           (:bold . "B")
                           (:teletype . "TT")
                           (:heading-1 . "H1")
                           (:heading-2 . "H2")
...
(defconstant *emphasis*  `((:emphasis . "EM")
                           (:strong . "STRONG")
                           (:address . "ADDRESS")
...
(defvar *input-types* (make-hash-table :size 14.))
	...
	(error-p
	 (error 'http::bad-multipart-form-data-posted
		:format-string "File Upload: Unknown directory index ~S.~
                              ~&The may client have submitted an obsolete file upload form with stale directory information.~
...
(defun %keyword-for-iso-character-description (string &optional (keyword-package http::*keyword-package*))
  (loop with delimiters = '(#\, #\space #\) #\()
        with l fixnum = (length string)
        for s = (position-if-not #'(lambda (x) (member x delimiters)) string :start (or e 0) :end l)
        for e = (and s (or (position-if #'(lambda (x) (member x delimiters)) string :start s :end l) l))
        while (and s e)
        when result
        collect "-" into result
        collect (string-upcase (subseq string s e))
        into result
        while (< (the fixnum e) l)
        finally (return (intern (apply #'concatenate 'string result) keyword-package))))

(defun %define-iso-readtable (spec &aux (keyword-package http::*keyword-package*))
  (flet ((initialize-entry (array name-table idx char name description)
           (let ((keyword (%keyword-for-iso-character-description description keyword-package)))
...
(defun %unused-iso-character-code-p (char-code)
  (or (<= 11 char-code 31)
      (<= 127 char-code 159))) ;html 4.0.1 adds 160
    ...
    (with-fast-array-references ((array *iso-readtable* array))
      (loop with type-idx = (ecase type
	    ...
	    when (and (not (%unused-iso-character-code-p idx))
		      ...
		      (http::string-search substring (ensure-string current-item)))
...
(define describe-iso-character (char-code &optional (stream *standard-output*)
  ...
  (cond ((%unused-iso-character-code-p char-code)
         (format stream "~&~3,,D~80TUnused" char-code))
...
(define-iso-readtable
  ...
  ;;         &#127; - &#159     Unused
  ;; Added HTML 4.0.1 ISO char names for 160-191 -- JCMa 10/29/2006 
  ((160 "nbsp")     "no-break space") ;html4.01
  ...
  ((168 "uml")      "Umlaut (dieresis)")
  ((169 "copy")     "Copyright")
  ((170 "ordf")     "Feminine ordinal")
  ((171 "lauo")     "Left angle quote, guillemotleft")
  ((172 "not")      "Not sign")
  ((173 "shy")      "Soft hyphen")
  ...
  ((187 "raquo")    "Right angle quote, guillemotright")
  ((188 "fraq14")   "Fraction one-fourth")
  ...
  ((215 "times")              "Multiple Sign") ;added char name for html 4.0.1
  ...
  ((247 "divide")              "Division Sign") ;added char name for html 4.0.1
