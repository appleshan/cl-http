...
;;; (C) Copyright 1994-2000, 2003, 2005-2006, John C Mallery.
;;;     All Rights Reserved.
;;;
;;;------------------------------------------------------------------- 
;;;
...
(define-condition search-url-form-encoding
  ...
  (:report (lambda (condition stream)
	     condition				;ignore
	     (format stream "URL form encoding error"))))
...
(define-condition search-syntax-not-defined-for-scheme
    (scheme-parsing-error)
  ((scheme :initarg :scheme :reader scheme))
  (:report (lambda (condition stream)
             (format stream "Scheme Error: Search arguments are not defined for the ~D scheme." 
                     (scheme condition)))))
...
(defvar *scheme-maximum-length* 6
  "The maximum length of a URL scheme, not including the delimiting colon.")

(defvar *scheme-alist* nil
  "An alist of (scheme-keyword scheme-scheme scheme-length scheme-prefix).")

(defun %register-scheme-formats (scheme)
  (check-type scheme keyword)
  (let ((entry (assoc scheme *scheme-alist*))
        scheme-string scheme-prefix)
    (unless entry
      (setq *scheme-alist* (nconc *scheme-alist*
                                  `((,scheme
                                     ,(setq scheme-string (string-downcase (symbol-name scheme)))
                                     ,(length scheme-string)
                                     ,(setq scheme-prefix (concatenate 'string scheme-string (case scheme
                                                                                               ((:news :mailto) ":")
                                                                                               (t "://"))))
                                     ,(length scheme-prefix))))))))

(defun %unregister-scheme-formats (scheme)
  (check-type scheme keyword)
  (let ((entry (assoc scheme *scheme-alist*)))
    (when entry
      (setq *scheme-alist* (delete entry *scheme-alist*)))))

(defun known-url-scheme-p (scheme &optional (error-p t))
  "Returns non-null if scheme is a known URL scheme."
  (cond ((member scheme *scheme-alist* :key #'car) t)
        (error-p
         (error "Unknown scheme, ~S." scheme))
        (t nil)))

;; see also canonicalize-url
(defun %scheme-prefix-for-protocol (protocol &optional (error-p t))
  "Returns a scheme prefix for PROTOCOL, which must be a keyword."
  (declare (values scheme-prefix scheme-prefix-length))
  (let ((entry (assoc protocol *scheme-alist*)))
    (cond (entry 
           (values (fourth entry) (fifth entry)))
          (error-p
           (error "PROTOCOL, ~S, is not a protocol with a known URL scheme." protocol))
          (t nil))))

(defun %scheme-for-protocol (protocol &optional (error-p t))
  "Returns the URL scheme string for PROTOCOL, which must be a keyword."
  (declare (values scheme scheme-length))
  (let ((entry (assoc protocol *scheme-alist*)))
    (cond (entry 
...
(defun %protocol-for-scheme (string  &optional (start 0) (end (length string)) no-colon-search-p (error-p t))
  "Returns the PROTOCOL for the URL scheme denoted by STRING."
  (declare (fixnum end))
  (let ((e (cond (no-colon-search-p end)
                 ((char-position #\: string start (min end *scheme-maximum-length*)))
                 (error-p (error "No colon found in STRING, ~S." (subseq string start end)))
                 (t nil))))
    (when e
      (loop for (protocol scheme scheme-length) in *scheme-alist*
            when (%STRING-EQUAL string scheme start e 0 scheme-length)
            return protocol
            finally (if error-p
                        (error "STRING, ~S, is not a URL scheme with a known protocol." (subseq string start end))
                      (return nil))))))
...
(defun register-scheme (scheme parser)
  ...
  (let ((entry (assoc scheme *scheme-parsers* :test #'string-equal)))
    ...
    (%register-scheme-formats (http:symbolize scheme http::*keyword-package*))
    *scheme-parsers*))
...
(defun %get-scheme (url-string &optional (start 0) (end (length url-string)))
  (declare (fixnum start end))
  (let* ((e (min (+ start *scheme-maximum-length* 1) end))
         (s (char-position #\: url-string start e)))
    ;;; I assume char-position can be nil, ecl does not like this declaration
    (declare (fixnum #-ecl s e))
    (subseq url-string s e)))
...
(define scheme-prefix-match-p (url-string schemes &optional (start 0) (end (length url-string)))
  "Returns non-null if URL carries scheme prefix that matches any of SCHEMES.
Schemes is either a cons of strings or a string. Comparison is case insensitive.
Returns the matching scheme."
  (declare (fixnum start end)
           (values matching-scheme scheme-end-index))
  (let ((pos (char-position #\: url-string start (min (the fixnum (+ start *scheme-maximum-length* 1))
                                                      end))))
    (values (etypecase schemes
              (string
               (when (string-equal url-string schemes :start1 start :end1 pos)
                 schemes))
              (cons 
               (loop for scheme in schemes
                     when (string-equal url-string scheme :start1 start :end1 pos)
                     return scheme
                     finally (return nil))))
            pos)))
...
(define fully-qualified-url-string-p (url-string &optional (start 0) (end (length url-string)))
  "Returns non-null if URL-STRING is a fully qualified URL string.
Returns the protocol keyword for the URL scheme if present."
  (declare (fixnum start end))
  (with-fast-array-references ((url-string url-string string))
    (let* ((colon (char-position #\: url-string start end)))
      (when (and colon
                 (< (+ 2 (the fixnum colon)) end)
        ...
        (cond ((string-equal "http" url-string :start1 0 :end1 4 :start2 start :end2 colon)
              ...
              ((string-equal "ftp" url-string :start1 0 :end1 3 :start2 start :end2 colon)
               :ftp)
              (t (let ((scheme (http::%tokenize-header-keyword url-string start colon)))
                   (when (known-url-scheme-p scheme)
                     scheme))))))))
  ...
  (flet ((bad-chars-in-host-p (host start end)
    ...
    (cond ((numeric-hostname-p host 0 len)
	  ...
	  (t (cond (*qualify-unqualified-domain-names*
		   ...
		   #+(or Allegro CCL-5.0 LispWorks CMU scl clisp ecl cormanlisp sbcl clozure-common-lisp)
...
(defun %root-context-string (url-string &optional (start 0) (end (length url-string)))
  "Returns the context string for URL-STRING."
  (multiple-value-bind (s e)
      (get-host-port-indices url-string start end)
    (let ((s1 (http::fast-position-if #'http::white-space-char-p url-string start s t)))
      (subseq url-string (if s1 (1+ s1) start) e))))
...
;; Recognize SGML entity references for & that may appear when a client fails to decode SGML
;; character references in a search URL.  
(defun %tokenize-ampersand-delimited-search-keyword (string &optional (start 0) (end (length string)))
  (declare (fixnum start end))
  (let ((char-entity-offset (+ start 4))
        real-start)
    (cond ((or (http::%string-equal "amp;" string 0 4 start char-entity-offset)
               (http::%string-equal "#38;" string 0 4 start char-entity-offset))
           (setq real-start char-entity-offset))
          (t (setq real-start start)))
    (http::%tokenize-form-query-keyword string real-start end)))

(defun %parse-search-info-as-query-alist (string field-delimiter query-tokenizer &optional (start 0) (end (length string)))
...
for use with form submission via the GET method."
  ...
  (flet ((handle-bad-form-encoding (string start end)
    ...
    (unless (= start end)
      (with-fast-array-references ((string string string))
	(loop for s1 fixnum = start then (1+ (the fixnum e2))
	      while (< s1 end)
	      for e1 fixnum = (or (char-position #\= string s1 end)
                                  (handle-bad-form-encoding string start end))
	      for s2 fixnum = (1+ (the fixnum e1))
	      for e2 fixnum = (or (char-position field-delimiter string s2 end) end)
	      for keyword = (funcall query-tokenizer string s1 e1)
	      for value = (when (and keyword (< s2 e2))
			    (string-unescape-special-chars string s2 e2))
              when keyword
	      collect `(,keyword ,value))))))

(defun %write-search-info-as-query-alist (search-alist field-delimiter &optional (stream *standard-output*) (escape-p *escape-search-urls*))
  (cond (escape-p
         (loop for ((keyword value) . alist) = search-alist then alist
               ...
               (when value
                 (let ((string (http:string-escape-special-chars value)))
                   (write-string string stream)))
               when alist
               do (write-char field-delimiter stream)
               while alist))
...
(declaim (inline parse-search-info-as-query-alist write-search-info-as-query-alist))

(defun parse-search-info-as-query-alist (string &optional (start 0) (end (length string)))
  "Parses the search component of a search URL according to HTML 3.2 URL encoding rules
for use with form submission via the GET method."
  (declare (values search-alist))
  (%parse-search-info-as-query-alist string #\& #'%tokenize-ampersand-delimited-search-keyword start end))

(defun write-search-info-as-query-alist (search-alist &optional (stream *standard-output*) (escape-p *escape-search-urls*))
  "Writes the search component of a search URL according to HTML 3.2 URL encoding rules
for use with form submission via the GET method."
  (%write-search-info-as-query-alist search-alist #\&  stream escape-p))

(declaim (inline parse-search-info-as-xml-safe-query-alist write-search-info-as-xml-safe-query-alist))

;; Use of ";" to delimit form fields recommended by html 4.0.1 spec
;; http://www.w3.org/TR/REC-html40/appendix/notes.html#h-B.2.2
(defun parse-search-info-as-xml-safe-query-alist (string &optional (start 0) (end (length string)))
  "Parses the search component of a search URL according to HTML 4.0 and XHTML URL encoding rules
for use with form submission via the GET method.

For HTML 4.0.1 and XHTML 1.0, this avoids interaction problems with use of & in SGML to delimit
character references."
  (declare (values search-alist))
  (%parse-search-info-as-query-alist string #\; #'http::%tokenize-form-query-keyword start end))

(defun write-search-info-as-xml-safe-query-alist (search-alist &optional (stream *standard-output*) (escape-p *escape-search-urls*))
  "Writes the search component of a search URL according to HTML 4.0 and XHTML URL encoding rules
for use with form submission via the GET method.
...
(defun search-url-form-encoding (string &optional (start 0) (end (length string)))
  "Returns the form encoding type for STRING."
  (declare (fixnum start end))
  (with-fast-array-references ((string string string))
    (loop with equal-sign-p and amp and semi-colon
          for idx fixnum upfrom start below end
          for char = (aref string idx)
          do (case char
               (#\& (if equal-sign-p
                        (return :form-ampersand-encoding)
                      (setq amp t)))
               (#\; (if equal-sign-p
                        (return :form-semi-colon-encoding)
                      (setq semi-colon t)))
               (#\= (cond (amp
                           (return :form-ampersand-encoding))
                          (semi-colon
                           (return :form-semi-colon-encoding))
                          (t (setq equal-sign-p t))))
               (#\+ (return :value-sequence)))
          finally (if equal-sign-p
                      (return :form-semi-colon-encoding)
                    (return :value-sequence)))))
...
(defun standard-parse-search-info (string &optional (start 0) (end (length string)))
  ...
  (declare (fixnum start end))
  (unless (= start end)
    (ecase (search-url-form-encoding string start end)
      (:form-semi-colon-encoding
       (parse-search-info-as-xml-safe-query-alist string start end))
      (:form-ampersand-encoding
       (parse-search-info-as-query-alist string start end))
      (:value-sequence
       (parse-search-info string start end)))))

(defvar *xml-safe-search-delimiters* t
  "Controls whether STANDARD-WRITE-SEARCH-INFO uses an XML-safe delimiter when writing search URL value
pairs.  When non-null, semi-colon is used as the delimited instead of ampersand, which interacts
with SGML character entity references. Pre-HTML 4.0 and XHTML 1.0 behavior may be obtained by
setting this to NIL.")
...
(defun standard-write-search-info (search-keys stream)
  ...
  (typecase (car search-keys)
    (cons
     (if *xml-safe-search-delimiters*
         (write-search-info-as-xml-safe-query-alist search-keys stream *escape-search-urls*)
...
(defun %protocol-for-url-string (url-string &optional (start 0) (end (length url-string)))
  (declare (fixnum start end)
           (optimize (speed 3)))
  (cond ((%string-equal "http://" url-string 0 5 start (min (+ start 5) end))
         :http)
        ((%string-equal "https://" url-string 0 6 start (min (+ start 6) end))
         :https)
        (t (multiple-value-bind (parser start-index)
               (get-scheme-parser url-string start end t)
             (declare (ignore parser))
             (if start-index
                 (http::%tokenize-header-value url-string start (1- (the fixnum start-index)))
               (error "Bad Syntax: URL-STRING must start with URL scheme, such as http://host.domain or https://host.domain."))))))
...
(defmethod protocol ((url-string string))
  (%protocol-for-url-string url-string))
...
;;;------------------------------------------------------------------- 
;;;
;;; VALUE CACHING FOR URLS
;;;

;; Record URL value cache keys for use by REMAP-URL-HOST
...
(defun %canonicalize-host-prefixed-url (scheme prefix prefix-length url-string start end &optional destructive-p)
  (declare (values canonical-url new-string-p)
	   (fixnum prefix-length start end))
  (flet ((downcase-char (string idx)
	   (setf (char string idx) (char-downcase (aref string idx))))
         (elide-port-p (scheme string start end)
           (let* ((port-end (char-position #\/ string start end nil))
                  (port (parse-integer string :start start :end (or port-end end))))
             (values (eql port (protocol-default-port scheme)) port-end))))
    (declare (inline downcase-char elide-port-p))
    ;; Ensure that URLs are in canonical form when doing parses or interns.
	...
	(%unescape-url url-string start end destructive-p)
      ...
      (let ((scheme-prefixed-p (string-equal prefix canonical-url :start1 0 :end1 prefix-length :start2 start :end2 (min prefix-length end)))
        ...
        (with-fast-array-references ((canonical-url canonical-url string))
          ;; downcase scheme
          (when scheme-prefixed-p
            (loop for idx fixnum upfrom start below (the fixnum (- prefix-length 3))
                  for char = (char canonical-url idx)
                  do (unless (lower-case-p char)
                       (setf (char canonical-url idx) (char-downcase char)))))
          ;; downcase host
          (loop for idx fixnum upfrom (if scheme-prefixed-p (the fixnum (+ start prefix-length)) start)  below end
                for char = (aref canonical-url idx)
                do (case char
                     (#\/ 
                      (setq host-delim idx)
                      (return))
                     (#\:
                      (multiple-value-bind (elide-p port-end)
                          (elide-port-p scheme canonical-url (1+ idx) end)
                        (cond
                         (elide-p
                          (multiple-value-bind (nstring copied-p)
                              (http::elide-string canonical-url idx (or port-end end) start end destructive-p)
                            (cond (copied-p
                                   (setq canonical-url nstring
                                         start 0
                                         end (length canonical-url)))
                                  (t (setq end (- end (- (the fixnum port-end) idx)))))
                            (when port-end
                              (setq host-delim idx))))
                         (port-end
                          (setq host-delim port-end)))
                        (return)))
                     (t (unless (lower-case-p char)
                          (setf (char canonical-url idx) (char-downcase char))))))
          ;; downcase any escapes
...
(defmacro url-class-accessor-form (scheme class-type)
  "Used inside WITH-URL-CLASS-MAP to obtain the class passed to make-instance."
...
(defvar *default-uri-table-size* 100
...
(initialize-standard-uri-universe)
		...
		(defun ,parser-name (url &optional (start 0) end (if-does-not-exist :create))
		  ...
		  (flet ((create-url (,url ,start ,end interned-p)
			...
			(multiple-value-bind (canonical-url new-url-p)
			    (canonicalize-url ,(intern prefix http::*keyword-package*) ,url ,start ,end)
...
(defun parse-url (url &optional (start 0) end (if-does-not-exist :create)) 
  ...
  (declare (optimize (speed 3))
           (values url-object newly-interned-p))
  (etypecase url
    (string
     (let (len start-idx end-idx)
       (flet ((end ()
                (or len (setq len (or end (length url)))))
              (whitespace (char)
                (member char '(#\space #\tab))))
         (declare (inline end whitespace))
         (if (and (setq start-idx (http::fast-position-if #'alpha-char-p url start (end) nil))
                  (setq end-idx (and start-idx (1+ (the fixnum (http::fast-position-if-not #'whitespace url start-idx (end) t))))))
             (funcall (get-scheme-parser url start-idx end-idx) url start-idx end-idx if-does-not-exist)
           (error 'no-scheme-found :url-string url)))))
    (url
...
(defun protocol-default-port (protocol)
  "Returns the default port used by PROTOCOL."
  (ecase protocol
    (:http 80)
    (:https 443)
    (:FTP 21)))

(define-generic locally-serving-url-p (url)
  (:documentation "Returns non-null when the server is serving url locally."))

(defmethod locally-serving-url-p ((url host-port-mixin))
  (and (url:local-url-p url)
       (eql (url:host-port url) http:*standard-http-port*)
       (eql (url:scheme url) http:*standard-protocol*)))

(defmethod locally-serving-url-p ((url string))
  (let ((protocol http:*standard-protocol*)
        (url-length (length url)))
    (multiple-value-bind (scheme-prefix protocol-length)
        (%scheme-prefix-for-protocol protocol)
      (cond ((%string-equal scheme-prefix url 0 protocol-length 0 (min protocol-length url-length))
             (multiple-value-bind (start end)
                 (get-host-port-indices url)
               (let* ((pidx (char-position #\: url start end t))
                      (port (if pidx 
                                (parse-integer url :start (1+ (the fixnum pidx)) :end end)
                              (protocol-default-port protocol))))
                 (and (eql port http:*standard-http-port*)
                      (or (%string-equal url (local-host-domain-name) start (or pidx end))
                          (http::virtual-host-string-p port url start (or pidx end)))))))
            (t nil)))))

(define-generic exported-p-url (url)
   (:documentation "Returns non-null if the URL is exported."))

(defmethod exported-url-p ((url url:translation-method-mixin))
  (and (slot-boundp url 'url:translation-method)
       (slot-value url 'url:translation-method)))

(defmethod exported-url-p (url)
  (declare (ignore url))
  nil)
...
(defmethod compute-name-string ((uri authority))
  (with-output-to-string (stream)
    (write-host-port-string uri stream)))
...
If compute-p is non-null, this computes it from the parsed data."))
...
(defmethod name-string ((uri authority) &optional compute-p)
  (with-slots (name-string) uri
    (cond ((or (null name-string) compute-p)
           (prog1 (setq name-string (compute-name-string uri))
             (when compute-p
               (clear-cached-values uri))))
          (t name-string))))
...
(defmethod name-string ((pathname pathname) &optional compute-p)
  (declare (ignore compute-p))
  (namestring pathname))

(define-generic external-name-string (http-url &optional compute-p)
  (:documentation "Returns the external name string for URL.
This means remapping the context as specified by HTTP:DEFINE-URL-CONTEXT-REMAPPINGS.
If compute-p is non-null, this recomputes it, otherwise returns the cached value."))

(defmethod external-name-string ((url http-url) &optional compute-p)
  (with-value-cached (url :external-name-string :recompute-p compute-p)
    (http::url-string-remapping-context url)))

#|
(define-generic escaped-name-string (url &optional compute-p)
  (:documentation "Returns the escaped name string for the URL."))

(defmethod escaped-name-string ((url host-port-mixin) &optional compute-p)
  (with-value-cached (url :escaped-name-string :recompute-p compute-p)
    (http:string-escape-special-chars (name-string url compute-p))))
|#

(defun %relative-name-string-indices (url-string start end)
  (declare (fixnum start end)
	   (string url-string))
  (let* ((pos1 (or (string-search= "://" url-string 0 3 start (min end (+ start 3 (the fixnum *scheme-maximum-length*))))
...
(defmethod relative-name-string ((url host-port-mixin) &optional compute-p)
...
#|
(define-generic escaped-relative-name-string (url &optional compute-p)
  (:documentation "Returns the escaped name string for the URL without the scheme or host/port prefix."))

(defmethod escaped-relative-name-string ((url host-port-mixin) &optional compute-p)
  (with-value-cached (url :escaped-relative-name :recompute-p compute-p)
    (http:string-escape-special-chars (relative-name-string url compute-p))))

|#
(defgeneric relative-name-string-for-locally-served-url (url)
  (:documentation "Returns a relative name string when URL is locally served."))

(defmethod relative-name-string-for-locally-served-url ((url host-port-mixin))
  (if (locally-serving-url-p url)
      (relative-name-string url)
    (name-string url)))

(defmethod relative-name-string-for-locally-served-url ((url string))
  (cond ((and (scheme-prefixed-url-p url)
              (locally-serving-url-p url))
         (relative-name-string url))
        (t url)))
...
(defmethod write-relative-name-string ((url host-port-mixin) &optional (stream *standard-output*))
...
(defmethod relative-path-string ((url host-port-mixin) &optional compute-p)
...
(defmethod relative-path-match-p ((url host-port-mixin) (path host-port-mixin))
  (%relative-path-match-p (relative-path-string path) (relative-path-string url)))

(defmethod relative-path-match-p ((url host-port-mixin) (path string))
...
(defun swap-url-context (url-string new-context &optional buffer)
  "Replaces the scheme://host:port for URL-STRING with NEW-CONTEXT.
URL-STRING must be fully specified. If BUFFER is supplied, the new url
is copied into it and the fill pointer set."
  (let* ((url-string-length (length url-string))
         (new-context-length (length new-context))
         size new-url-string)
    (url::with-relative-name-string-indices (url-string 0 url-string-length)
      (setq size (+ new-context-length (- end start))
            new-url-string (or buffer (make-string size)))
      (copy-vector-portion new-context 0 new-context-length new-url-string 0 new-context-length)
      (copy-vector-portion url-string start end new-url-string new-context-length size)
      (when buffer
...
(define-generic map-url-inferiors (url function)
  (declare (values url))
  (:documentation "Maps FUNCTION over the inferiors of URL, but not including URL.
Works only with path URLs.  The application of FUNCTION is in no particular order."))

(defmethod map-url-inferiors  ((url path-mixin) function)
  (let* ((protocol (protocol url))
         (host (host-string url))
         (port (port url))
         (name (name-string url))
         (end (length name))
         s1 e1 len1)
    (with-relative-name-string-indices (name 0 end)
      (setq s1 start
            e1 end
            len1 (- end start)))
    (flet ((fctn (url-string url-obj)
             (when (and (eql protocol (protocol url-obj))
                        (eq port (port url-obj))
                        (equalp host (host-string url-obj)) ;host can be case insensitive
                        (not (eq url url-obj)))
               (with-relative-name-string-indices (url-string 0 (length url-string))
                 (when (and (< len1 (- end start))
                            (%string= name url-string s1 e1 start (+ start len1)))
                   (funcall function url-obj))))))
...
(defun coerce-url-string* (url &optional (escape-search-url-p *escape-search-urls*)
  ...
  "Coerces URL into the correct string according to the arguments.
See coerce-url-string."
  (if downcase-p
      (string-downcase (coerce-url-string url escape-search-url-p relativize))
      (coerce-url-string url escape-search-url-p relativize)))
...
(defmethod coerce-url-string ((url search-mixin) &optional (escape-search-url-p *escape-search-urls*)
  ...
  (flet ((get-relative-url-based-on-context (context url)
    ...
    (cond (escape-search-url-p
	   (ensure-escaped-search-url (call-next-method)))
...
(define-macro without-url-modification (&body body)
  "Prevents URLs from being relativized or escaped within the scope of BODY."
  `(let ((*relativize-urls* nil) 
         (*escape-search-urls* nil))
     ,@body))
...
(defmethod change-host ((url host-mixin) (host-name string) (port integer))
  ...
  (setf (host-string url) host-name
        (port url) (if (= port (standard-port url)) nil port)
        (%host-object url) nil)
  (name-string url t)
  (register url))
...
(define-generic write-name (uri &optional stream)
  (:documentation "Writes the URIs print name on STREAM."))

(defmethod write-name ((uri uri) &optional (stream *standard-output*))
  (write-string (name-string uri) stream))

(defmethod write-name ((uri string) &optional (stream *standard-output*))
...
(defmethod documentation ((url documentation-mixin) #-ANSI-Documentation &optional doc-type)
  ...
  (let* ((len (length string))
    ...
    (cond ((or (null pos) (= (1+ (the fixnum pos)) len))
          ...
          (t (let* ((idx (1+ (the fixnum pos)))
               ...
               (concatenate (list 'array (array-element-type string) '(*))
                            stem suffix))))))
...
(defmethod relative-name-string-with-unescaped-search-suffix ((url host-port-mixin) &optional recache-p)
  (relative-name-string url recache-p))
...
(defmethod search-suffix ((url search-mixin) &optional include-suffix-delimiter-p)
  (with-slots (name-string) url
    (let* ((len (length name-string))
      ...
      (declare (fixnum len))
      (if (and pos (/= (the fixnum pos) len))
          (subseq name-string (if include-suffix-delimiter-p pos (1+ (the fixnum pos))) len)
          ""))))
...
(defconstant *url-schemes-supporting-search-syntax* '(:http :https :wais :gopher)
  "The URL schemes that support search syntax.")

(declaim (inline check-scheme-search-support))

(defun check-scheme-search-support (scheme)
  (unless (member scheme *url-schemes-supporting-search-syntax*)
    (error 'search-syntax-not-defined-for-scheme :scheme scheme)))

(defun %make-url-string (scheme host port path name extension search-args)
  (flet ((host-string (host scheme)
           (multiple-value-bind (scheme-prefix scheme-prefix-length)
               (%scheme-prefix-for-protocol scheme)
             (let* ((l (length host))
                    (pos1 (if (string-equal scheme-prefix host
                                            :start1 0 :end1 scheme-prefix-length
                                            :start2 0 :end2 (min scheme-prefix-length l))
                              scheme-prefix-length))
                    (pos2 (or (char-position #\: host (or pos1 0) l t)
                              (char-position #\/ host (1- (the fixnum l)) l t))))
               (declare (fixnum l))
               (values (if (or pos1 pos2) (subseq host (or pos1 0) (or pos2 l)) host)
                       scheme-prefix))))
         (name-spec ()
           `(,name
    ...
    (declare (dynamic-extent #'host-string)
             (inline name-spec))
    (check-scheme-search-support scheme)
    (loop for item in path
          ...
          (when host
            (etypecase port
              (null)
              (number
               (unless (= port (protocol-default-port scheme))
                 (push (write-to-string port :base 10 :escape nil) path-spec)
                 (push ":" path-spec)))
              (string
               (push port path-spec)
               (push ":" path-spec)))
            (multiple-value-bind (host-string scheme-prefix)
                (host-string host scheme)
              (push host-string path-spec)
              (push scheme-prefix path-spec)))
          (return (apply #'http:string-append path-spec)))))

(define-macro make-url-string (&key (scheme :http) host port path name extension search-args search-p)
  "Makes an HTTP string from the components."
  `(%make-url-string ,scheme ,host ,port ,path ,name ,extension ,(or search-args (when search-p 't))))

(define make-url-string-from-pathname (scheme pathname &optional directory-file-p host port extension)
  "Returns an URL string with scheme, SCHEME, for pathname using PATHNAME host as the host.
If DIRECTORY-FILE-P, name and type of pathname are ignored.  HOST overides
the pathname host.  PORT  specifies port for the URL."
  (flet ((host-string (pathname)
    ...
    (let* ((path (pathname pathname))
           (host-string (or host (host-string path)))
           (directory (pathname-directory path))
           (name (pathname-external-name-string (pathname-name path))))
      (if directory-file-p
          (make-url-string :scheme scheme
                           :host host-string
                           :port port
...
(define-generic intern-pathname-as-url (pathname &key protocol host port directory-file-p if-does-not-exist)
  (declare (values url intern-status))
  (:documentation "Interns a URL for protocol, PROTOCOL, based on PATHNAME.
If DIRECTORY-FILE-P, name and type of pathname are ignored.
HOST overides the pathnamehost.  PORT specifies the port for PROTOCOL."))

(defmethod intern-pathname-as-url ((pathname pathname) &key (protocol http:*standard-protocol*) host port 
                                   (directory-file-p (pathname-directory-p pathname)) (if-does-not-exist :create))
  (let ((spec (make-url-string-from-pathname protocol pathname directory-file-p host port)))
...
(defmethod intern-pathname-as-url-inferior ((pathname pathname) (url path-mixin) &key extension if-does-not-exist
  ...
  (cond (directory-p
         (let* ((name (pathname-external-name-string (pathname-directory-most-specific-name pathname)))
                (string (http:string-append (url:name-string url) name "/")))
        ...
        (t (let* ((name (pathname-external-name-string (pathname-name pathname)))
                  (string (http:string-append (url:name-string url) name "." (or extension (pathname-type pathname)))))
             (declare (dynamic-extent name string))
             (url:intern-url string :if-does-not-exist if-does-not-exist)))))
...
This is the keyword for the primary EXTENSION defined by DEFINE-EXPORT-TYPE.  When
ERROR-IF-UNKNOWN-P is non-null, an error is signalled if the data type is unknown.  But, when
ERROR-IF-UNKNOWN-P is null, the data type :OCTET is returned when the data type is unknown."))
...
(defmethod data-type ((pathname pathname) &optional error-if-unknown-p)
  (let ((type (pathname-type pathname)))
    (%data-type-for-pathname-extension type error-if-unknown-p)))
...
(defgeneric content-type-keyword (url-object &optional error-if-unknown-p)
  (:documentation "Returns a keyword that denoted the primary pathname extension for export.
This keyword corresponds to a MIME content type."))

(defmethod content-type-keyword ((object object-mixin) &optional error-if-unknown-p)
  (let ((extension (extension object)))
    (or (http::primary-pathname-extension extension error-if-unknown-p)
        :octet)))

(defmethod content-type-keyword ((object path-mixin) &optional error-if-unknown-p)
  (declare (ignore error-if-unknown-p))
  :html)
  ...
  (let ((translation-method (translation-method url))
    ...
    (cond (translation-method
          ...
          ((and (setq extension (extension url))
                (setq extension (http:primary-pathname-extension extension)))
           (http:%mime-content-type-spec extension))
...
;; Default method handles non-http/https schems
(defmethod mime-content-type-spec ((url object-mixin))
  (http:%mime-content-type-spec (content-type-keyword url nil)))
...
(defmethod copy-mode ((url url)) :binary)
...
(defmethod copy-mode ((url object-mixin))
  (let ((ext (extension url))
    ...
    (cond ((and ext
                (setq content-type-keyword (http:primary-pathname-extension ext nil))
                (%content-type-copy-mode content-type-keyword t)))
          (t :binary))))

(define-generic element-type (url)
  (:documentation "Returns an ANSI CL type specifier for the resource."))

(defmethod element-type ((url url))
  (http::copy-mode-element-type (copy-mode url)))
...
(defmethod export-type ((url object-mixin))
  (with-slots (extension) url
    (http:export-type-for-pathname-type (or extension :octet))))
...
(define make-file-url-string (host-string dir &optional name extension)
  "Returns an FILE URL string."
  (let ((args `(,.(loop for item in dir
			...
			collect item)
                  ...
                  ,.(case extension
                      ...
                      (:wild (list "*"))
                      (t `("." ,extension))))))
    (apply #'http:string-append "file://" host-string args)))

;; Ports may specialize this to handle vagaries of their pathname implementation
(define-generic pathname-file-url-string (pathname &optional force-directory-p)
  (:documentation "Returns an FILE URL string for PATHNAME."))

(defmethod pathname-file-url-string (pathname &optional force-directory-p &aux (path (pathname pathname)))
  (let ((dir (pathname-directory path))
        (name (pathname-external-name-string (pathname-name path)))
        (extension (pathname-type path))
        (host (pathname-host path)))
    (case host ;; non Genera may come up :unspecific
      ((:unspecific nil) (setq host (local-host-domain-name))))
    (cond (force-directory-p
	   (if name
	       (make-file-url-string (host-http-name host) `(,@(cdr dir) ,name))
             (make-file-url-string (host-http-name host) (cdr dir))))
	  (t (make-file-url-string (host-http-name host) (cdr dir) name extension)))))

(define-generic file-url-pathname (url)
  (:documentation "Returns a pathname for the FILE URL."))

(defmethod file-url-pathname ((url file-directory))
  (with-slots (path) url
    (make-pathname :host #+Genera (host-for-pathname url) #-Genera nil
                   :directory (cons :absolute path))))

(defmethod file-url-pathname ((url file-pathname))
  (with-slots (path object extension #+Genera version) url
    (make-pathname :host #+Genera (host-for-pathname url) #-Genera nil
                   :directory (cons :absolute path)
                   :name object
...
(defmethod user-id-and-password (url)
  (declare (ignore url))
  nil)
...
(defmethod set-expiration-function ((expiration-mixin expiration-mixin) (type (eql :never)) &rest arguments)
  ...
  (with-slots (expiration-function) expiration-mixin
    (setf expiration-function #'(lambda (url)
                                  (declare (ignore url))
                                  (the integer (+ *one-year-interval*  (get-universal-time)))))))
...
(defmethod set-expiration-function ((expiration-mixin expiration-mixin) (type (eql :interval)) &rest arguments)
  (with-slots (expiration-function) expiration-mixin
    (destructuring-bind (argument) arguments
      ...
      (setf expiration-function #'(lambda (url)
                                    (declare (ignore url))
                                    (the integer (+ (the integer (get-universal-time)) argument)))))))
  ...
  (with-slots (max-age-function) expiration-mixin
    (destructuring-bind (argument) arguments
      ...
      (setf max-age-function #'(lambda (url)
                                 (declare (ignore url))
                                 (- (the integer (get-universal-time))
...
(defmethod get-image-size ((url http-object) (translation-method (eql :png-image)) &optional recompute-p)
  (declare (ignore recompute-p))
  (with-slots (pathname) url
    (png-image-dimensions pathname t)))
...
(defmethod clear-response-times ((url http-url))
  (setf (slot-value url 'cpu-time)     0
        (slot-value url 'elapsed-time) 0
        (slot-value url 'n-requests)   0))
...
#+(or ANSI-CL Draft-ANSI-CL-2)
(defmethod make-load-form ((mail-address mail-address) &optional environment)
  (declare (ignore environment))
  (with-slots (user-id host-string personal-name original-string) mail-address
    `(create-internet-mail-address ,user-id ,host-string ,personal-name ,original-string)))
...
(defun parse-internet-mail-address (string &optional (start 0) (end (length string)))
  ...
  (flet ((parse-address (string start end)
	   ...
	   (let* ((at-pos (char-position #\@ string start end t))
	     ...
	     (declare (type (or fixnum null) at-pos at1))
	     (when at-pos
...
(defun %make-email-address (user-id host-string)
  (let* ((l1 (length user-id))
	 ...
	 (string (make-array l3 :element-type *standard-character-type*)))
...
(defgeneric full-email-address (mailto-url &optional recompute-p)
  (declare (values full-email-address))
  (:documentation "Returns the email address including the personal name for mailto-url."))
