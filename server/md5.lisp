...
;;; Added MD5-DIGEST-INTEGER  9/12/03 -- JCMa.
...
(defmacro 32-identity (num)
  #+genera
  `(si:%logldb (byte 32. 0) ,num)
  #-genera
  `,num)
)
...
;;; number <<< by
(defun 32-left-rot-function (number by)
  (let ((break (- 32 by)))
...
(defmacro 32-left-rot (number by)
  #+genera
  `(si:rot ,number ,by)
  #-genera
  `(32-left-rot-function ,number ,by))
...
(defmacro initialize-md5-state (a b c d)
  `(setf ,a (32-identity #x67452301)
         ,b (32-identity #xefcdab89)
         ,c (32-identity #x98badcfe)
         ,d (32-identity #x10325476)))
...
(proclaim '(inline md5-function-f md5-function-g md5-function-h md5-function-i))
...
(eval-when (compile load eval)
  (defvar *random-sine-table*
          (make-array 64
                      ...
                      :initial-contents
                      (map 'list #'(lambda (x) (32-identity x))
...
(defmacro with-stack-array ((var dimensions &rest keyargs) &body body)
  #+Genera
  `(sys:with-stack-array (,var ,dimensions ,@keyargs)
     (declare (sys:array-register ,var))
     ,@body)
  #-Genera
...
(defmacro md5-digest-byte-integer (a b c d)
...
;; Platform-specific optimized version in http:mcl;server;md5.lisp
#-(or MCL LispWorks)
...
(defun md5-digest-equal (digest1 digest2)
  "Compares two MD5 digests and returns non-null when they are equivalent."
  (cond
...
(defun md5-digest-integer (string &optional (start 0) end)
  ...
  (multiple-value-bind (a b c d)
      (md5-encode-string string start end)
    (md5-digest-byte-integer a b c d)))

(defgeneric md5-encode-file (pathname &optional element-type start limit))

(defmethod md5-encode-file (pathname &optional (element-type '(unsigned-byte 8)) start limit)
  (with-open-file (stream pathname :direction :input
                          :element-type element-type :if-does-not-exist :error)
    (md5-encode-stream stream element-type start limit)))

;; Specialize this method for fast MD5 for stream data sources (e.g., files).
;; See also other platform-specific ways to optimize MD5
