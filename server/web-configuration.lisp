...
;;;
;;; (C) Copyright 1996-97, 2006, Christopher R. Vincent and John C. Mallery
;;;     All Rights Reserved.
;;;
;;;------------------------------------------------------------------- 
;;;
          ...
          (with-section-heading ("Overview" :stream stream)
            (with-enumeration (stream :enumerate :type :capital-letters)
              (loop for type in types
                    for ref across "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                    do (flet ((write-ref (stream)
                                (princ ref stream)))
                         (declare (dynamic-extent #'write-ref))
                         (enumerating-item (stream)
                           (note-anchor (preference-type-display-string type) :local-reference #'write-ref :stream stream)))))))))
    ...
    (with-enumeration (stream :definition)
      (loop for type in types
            ...
            do (flet ((note-preference-type (stream)
                        (flet ((write-tag (stream)
                                 (princ ref stream))
                               (write-heading (stream)
                                 (fast-format stream "~C. ~A" ref head)))
                          (declare (dynamic-extent #'write-tag #'write-heading))
                          (with-anchor-noted (:tag #'write-tag :stream stream)
...
(defun %compute-configure-server (url stream)
  (flet ((write-heading (stream)
           (fast-format stream "Configure CL-HTTP Server at ~A" (local-context))))
    (with-cl-http-html-document (:declare-dtd-version-p :transitional :stream stream)
      (with-document-look-preamble (:stream stream)
        (declare-title #'write-heading :stream stream))
      (with-document-look-body (:heading #'write-heading :header-class "header.cl-http" :body-class "body.cl-http"
                                :footer-class "footer.cl-http" :stream stream)
        (with-rendition (:italic :stream stream)
          (fast-format stream "This form allows maintainers to perform server configuration over the Web."))
        (with-paragraph (:stream stream)
...
(defun report-configuration-error (url stream &key preference input-string)
  ...
  (with-cl-http-html-document (:declare-dtd-version-p :transitional :stream stream)
    ...
    (with-document-look-body (:heading "Configuration Error" :header-class "header.cl-http" :body-class "body.cl-http"
                              :footer-class "footer.cl-http" :stream stream)
      (with-paragraph (:stream stream) 
...
(defun export-web-configuration-interface (&key (secure-subnets (list (local-host-ip-address)))
                                                (authentication-realm :server) (capabilities :webmasters)
                                                (protocol *standard-protocol*) (host (local-host-domain-name)) (port *standard-http-port*))
...
does not protect from man-in-the-middle attacks."
  (let ((realm (and authentication-realm (intern-realm authentication-realm :if-does-not-exist :soft))))
    (export-url #u("/cl-http/maintenance/configure-server.html" :host host :port port :protocol protocol)
                :html-computed-form
