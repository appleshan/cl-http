;;;   -*- Mode: LISP; Package: script; BASE: 10; Syntax: ANSI-Common-Lisp; Default-Character-Style: (:FIX :ROMAN :NORMAL);-*-
;;;
;;; (c) Copyright 1996, 2005, John C. Mallery
;;;     All Rights Reserved.
;;;
;;;------------------------------------------------------------------- 
;;;
...
(defclass script
  ...
  ((name :initarg :name :reader script-name)
   ...
   (displays-content-p :initform nil :initarg :displays-content-p :accessor script-displays-content-p) ;html 4.0.1 parameter
   (charset :initform nil :initarg :charset :accessor script-charset)) ;html 4.0.1 parameter
...
(defclass java-script1.2
  ...
  (:documentation "A JavaScript 1.2 program suitable for the Netscape 3.0 client."))

(defclass java-script1.3
          (java-script)
    ((language :initform :java-script1.3 :allocation :class))
  (:documentation "A JavaScript 1.3 program suitable for the Netscape 3.0 client."))

(defclass java-script1.4
          (java-script)
    ((language :initform :java-script1.4 :allocation :class))
  (:documentation "A JavaScript 1.4 program suitable for the Netscape 3.0 client."))

(defclass java-script1.5
          (java-script)
  ((language :initform :java-script1.5 :allocation :class))
  (:documentation "A JavaScript 1.5 program suitable for the Netscape 3.0 client."))
...
;; Suitable for Netscape 5.0
(define-script-language java-script1.3
                        :class java-script1.3
                        :content-type :javascript)

;; Suitable for Netscape 6.0
(define-script-language java-script1.4
                        :class java-script1.4
                        :content-type :javascript)

;; Suitable for Netscape 7.0
(define-script-language java-script1.5
                        :class java-script1.5
                        :content-type :javascript)
...
(defun %make-define-script-code (name language displays-content-p charset script-location documentation caller script)
  (let* ((name (string name))
         (language (http:symbolize language http:*keyword-package*)))
    `(let ((script (intern-script ',name ',language :url ,script-location :if-does-not-exist :create)))
       ,(if documentation
            `(setf (http:get-value script :documentation) ,documentation)
          `(http:remove-value script :documentation))
       (setf (script-displays-content-p script) ,displays-content-p
             (script-charset script) ,charset)
       ,.(%define-script-writer name language script)
       ,.(%define-script-caller name language caller)
       script)))

(define-macro define-script (name (language) &key documentation location displays-content charset caller script url)
  "Defines a script named NAME in the scripting language, LANGUAGE.
...
INTERN-SCRIPT, WRITE-SCRIPT."
  (%make-define-script-code name language displays-content charset (or location url) documentation caller script))

;;;------------------------------------------------------------------- 
;;;
;;; METHODS ON SCRIPTS
...
(define-generic script-charset (script)
  (:documentation "Returns the character set for SCRIPT source code."))

(define-generic script-displays-content-p (script)
  (:documentation "Returns non-null if SCRIPT displays content while loading or when its source is read by browsers ."))
...
(defmethod documentation ((script script) #-ANSI-documentation &optional doc-type)
  (declare (ignore doc-type))
  (http:get-value script :documentation))

;;;------------------------------------------------------------------- 
;;;
;;; NETSCAPE 2.0 SUPPORT
;;;

(define-generic ns2.0:declare-script (script stream &optional force-write-p)
...
(defmethod ns2.0:declare-script ((script script-writer-mixin) stream &optional force-write-p &aux url)
  (if (or force-write-p (null (setq url (script-url script))))
      (ns2.0:with-embedded-script ((script-language script) :stream stream)
        (funcall (script-writer script) script stream))
      (ns2.0:with-embedded-script ((script-language script) :base-code-url url :stream stream))))

(define-generic ns2.0:write-script (script stream &rest lambda-list)
  (:documentation "Writes the text of SCRIPT on STREAM.
LAMBDA-LIST is the set of argument required to write the script."))

(defmethod ns2.0:write-script ((script script-writer-mixin) stream &rest lambda-list)
  (declare (dynamic-extent lambda-list))
...
(define-generic script-html4.0-parameters (script)
  (declare (values media-type script-location display-content charset language))
  (:documentation "Returns values needed to write HTML scripts."))

(defmethod script-html4.0-parameters ((script script))
  (with-slots (content-type url displays-content-p charset language) script
    (values content-type url displays-content-p charset language)))

(define-generic html4.0:declare-script (script stream &optional force-write-p)
  (:documentation "Declares SCRIPT on STREAM.

Use to emit HTML containing either the text of the script or a reference to the URI
where the script is located.  When fORCE-WRITE-P is non-null, the script text is always
written to STREAM.

A script can only be declared in this way when it does not take write arguments.  When
write arguments are required, use WRITE-SCRIPT."))

(defmethod html4.0:declare-script ((script script) stream &optional force-write-p)
  (multiple-value-bind (media-type script-location display-content charset language)
      (script-html4.0-parameters script)
    (html4.0:with-script (media-type :script-location script-location :no-content (not display-content) 
                                     :charset charset :language language :stream stream)
      (when (or force-write-p (null script-location))
        (funcall (script-writer script) script stream)))))

(define-generic html4.0:write-script (script stream &rest lambda-list)
  (:documentation "Writes the text of SCRIPT on STREAM.
LAMBDA-LIST is the set of argument required to write the script."))

(defmethod html4.0:write-script ((script ns2.0::script) stream &rest lambda-list)
  (declare (dynamic-extent lambda-list))
  (multiple-value-bind (media-type script-location display-content charset language)
      (script-html4.0-parameters script)
    (html4.0:with-script (media-type :script-location script-location :no-content (not display-content) 
...
#|
NETSCAPE2.0:JAVA-SCRIPT [Class]: #<STANDARD-CLASS NETSCAPE2.0:JAVA-SCRIPT 119DB3A7>
NETSCAPE2.0:DEFINE-SCRIPT-LANGUAGE [macro]: (NAME &KEY CLASS CONTENT-TYPE)
NETSCAPE2.0:SCRIPT-WRITER-ARGLIST [function]: (SCRIPT)
DECLARE-SCRIPT [function]: (SCRIPT STREAM &OPTIONAL FORCE-WRITE-P)
NETSCAPE2.0:SCRIPT-LANGUAGE [function]: (SCRIPT)
NETSCAPE2.0:WRITE-RAW-SCRIPT [function]: (SCRIPT STREAM &REST LAMBDA-LIST)
NETSCAPE2.0:SCRIPT-NAME [function]: (SCRIPT)
NETSCAPE2.0:SCRIPT-CALLER-ARGLIST [function]: (SCRIPT)
NETSCAPE2.0:SCRIPT-CONTENT-TYPE [function]: (SCRIPT)
NETSCAPE2.0:WRITE-SCRIPT [function]: (SCRIPT STREAM &REST LAMBDA-LIST)
NETSCAPE2.0:UNINTERN-SCRIPT [function]: (SCRIPT)
NETSCAPE2.0:WITH-EMBEDDED-SCRIPT [macro]: ((LANGUAGE &KEY BASE-CODE-URL (STREAM (QUOTE *OUTPUT-STREAM*))) &BODY BODY)
NETSCAPE2.0:EXPORT-SCRIPT [function]: (SCRIPT &REST EXPORT-ARGS &KEY URL &ALLOW-OTHER-KEYS)
NETSCAPE2.0:SCRIPT-BYTES [function]: (SCRIPT)
NETSCAPE2.0:INTERN-SCRIPT [function]: (NAME LANGUAGE &KEY URL (IF-DOES-NOT-EXIST ERROR))
NETSCAPE2.0:DEFINE-SCRIPT [macro]: (NAME (LANGUAGE) &KEY DOCUMENTATION URL CALLER SCRIPT)
|#
...
(html4.0::define-script "Check_Integer_Bounds"
                        (java-script)
