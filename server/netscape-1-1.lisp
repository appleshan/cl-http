...
will get (reload) url after INTERVAL."
  ...
  (cond (url
	 (flet ((write-referesh-spec (stream)
		  (http::fast-format stream "\"~D; URL=~I\"" interval
				     (http::write-url-remapping-context url stream))))
	   (declare (dynamic-extent #'write-referesh-spec))
           (declare-meta-info #'write-referesh-spec :header :refresh :stream stream)))
...
replaces any calls to HTTP:WITH-SUCCESSFUL-RESPONSE or variants."
  (let ((boundary (http::mime-multipart-boundary-string :multipart-mixed-replace)))
    `(macrolet ((with-block ((stream &key (last-block-p nil last-block-supplied-p) 
                                     (force-output t)
                  ...
                  `(http::with-mime-multipart-block :multipart-mixed-replace
                       (,stream
                        :boundary ,,boundary
                        ,.(when last-block-supplied-p `(:last-block-p ,last-block-p))
                        :force-output ,force-output
                        ,.(when sleep-interval-supplied-p`(:sleep-interval ,sleep-interval))
