...
(defun sha-hexadecimal-encode (&rest sha-values)
  ...
  (macrolet ((hexdecimal-digit (hexdecimal position)
               `(char "0123456789abcdef" (ldb (byte 4 ,position) ,hexdecimal)))
             (next-idx (n)
               `(1+ (the fixnum ,n))))
    (let ((string (make-array 40 :element-type http::*standard-character-type*)))
      (www-utils:with-fast-array-references ((string string))
        (loop for item in sha-values
              ...
              do (setf (char string idx) (hexdecimal-digit item 4)
                       (char string (next-idx idx)) (hexdecimal-digit item 0))))
      string))) 
...
;; Platforms shoud specialize this method for higher performance.
