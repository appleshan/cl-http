...
;;; Copyright 1999-2001, 2005, John C. Mallery.
;;; All rights reserved.
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (defpackage tk1
    ...
    #+(and :mit-loop (or :ecl :clisp))(:shadowing-import-from :mit-loop "LOOP" "LOOP-FINISH")
  ...
  (ecase hash-mask
    (#b11011111
     (with-stack-substring (str string :start start :end end)
                           ...
                           (sxhash str)))
    (#b11111111
     (if (and (eql 0 start) (or (null end) (eql (the fixnum end) (length string))))
         (sxhash string)
         (with-stack-substring (str string :start start :end end)
                               (sxhash str))))))
...
as well as a threshold when the tokenizer table should be rehashed."
  ...
  (let* ((min-size (ceiling (* #+Genera filled-size
    ...
    (values modulus (floor (* (the fixnum modulus) 4) 5)))) 
...
(defun %rehash-tokenizer (tokenizer &optional size)
  ...
  (flet ((entry-valid-p (entry)
    ...
    (declare (inline entry-valid-p rebucket))
    (multiple-value-bind (modulus rehash-size)
