...
;;; (C) Copyright 1998, 2003, 2006-2007, John C. Mallery.
;;;     All Rights Reserved.
;;;
;;;------------------------------------------------------------------- 
;;;
...
(defun make-data-cache-array (resource size)
  ...
  (scl:make-array size :element-type '(unsigned-byte 8) :fill-pointer 0 :adjustable t :area *data-cache-area*))
...
(defun deinitialize-data-cache-array (resource array)
  (declare (ignore resource))
...
(defresource data-cache-array (size)
  :deinitializer deinitialize-data-cache-array
...
; new version without using www-utils:atomic-conditional-replacef, required for LispWorks6
(defmethod note-data-access :after ((universe data-universe-cache-mixin) (data-cache data-cache-cache-mixin)
                                    &optional access-time)
  (declare (ignore access-time))
  (with-lock-held ((data-universe-lock universe))
    (unless (data-cache-wired-p data-cache)
      (when (or (null (slot-value universe 'minimum-frequency-data-cache))
                (< (data-cache-reference-count (slot-value universe 'minimum-frequency-data-cache))
                   (data-cache-reference-count data-cache)))
        (setf (slot-value universe 'minimum-frequency-data-cache) data-cache)))
    data-cache))
    ...
    (loop with wait-whostate = (data-universe-revalidator-wait-whostate data-universe)
	  ...
	  doing (unless (data-universe-revalidator-work-p data-universe)
		...
		(let* ((start-time (get-universal-time))
		  ...
		  (declare (integer start-time finish-time next-revalidation))
...
(defgeneric cache-file-data (url)
  (:documentation "Adds URL's file data to the in-memory data cache."))

(defmethod cache-file-data ((url file-data-caching-object))
  (with-slots (data-universe dc-revalidation-interval dc-wired-p) url
    (cache-data data-universe (cached-pathname url)
                :revalidation-interval dc-revalidation-interval
                :wired-p dc-wired-p)))

(defgeneric decache-file-data (url)
  (:documentation "Removes URL's file data to the in-memory data cache."))

(defmethod decache-file-data ((url file-data-caching-object))
  (with-slots (data-universe pathname) url
    (decache-data data-universe pathname)))

(defgeneric initialize-file-data-cache (url data-cache-universe &key revalidation-interval wired-p)
  (:documentation "Initializes data cache for URL's file data using DATA-CACHE-UNIVERSE.
REVALIDATION-INTERVAL controls how frequently the in-memory data is revalidated against the on-disk data.
WIRED-P controls whether the data can be decached based on low frequency of access."))

(defmethod initialize-file-data-cache ((url url:file-data-caching-mixin) (data-cache-universe file-data-universe) 
                                       &key (revalidation-interval *data-universe-revalidation-interval*) wired-p)
  (check-type revalidation-interval integer)
  (with-slots (data-universe dc-revalidation-interval dc-wired-p) url
    (setq data-universe data-cache-universe
          dc-revalidation-interval revalidation-interval
          dc-wired-p wired-p)))

(defmethod initialize-file-data-cache :after ((url url:file-data-caching-object) (data-cache-universe file-data-universe) 
                                              &key revalidation-interval wired-p)
  (declare (ignore revalidation-interval wired-p))
  (cache-file-data url))

(defgeneric deinitialize-file-data-cache (url)
  (:documentation "Deinitializes data cache for URL's file data."))

;; Do nothing for no data caching URLs when mapping inferiors
(defmethod deinitialize-file-data-cache (url)
  url)

(defmethod deinitialize-file-data-cache ((url url:file-data-caching-object))
  (decache-file-data url))

(defmethod deinitialize-file-data-cache ((url url:file-data-caching-path))
  (map-url-inferiors url #'deinitialize-file-data-cache))

(defmethod deinitialize-file-data-cache :around ((url url:file-data-caching-mixin))
  (with-slots (data-universe dc-revalidation-interval dc-wired-p) url
    (when data-universe
      (prog1 (call-next-method url)
        (setq data-universe nil
              dc-revalidation-interval nil
              dc-wired-p nil)))))
