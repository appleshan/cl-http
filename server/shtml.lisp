...
;;;
;;; (c) Copyright  1997, 2000, 2005, John C. Mallery
;;;     All Rights Reserved.
;;;
...
SHTML-FUNCTION must be an fbound symbol."
  (check-type shtml-function (and symbol (satisfies symbol-fbound-p)))
  (let ((action (shtml-action-name shtml-function)))
    (setf (gethash action *shtml-action-table*) shtml-function)
    (unless (member shtml-function *shtml-actions*)
...
Actions are inserted into static HTML using the tag <!--#eval action=name -->.
Example: <!--#eval action=\"date-time\" format=\"iso\"-->
...
(define-header-keywords "eval" "include")
