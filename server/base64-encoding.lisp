...
(defun base64-encode-vector (byte-vector &key (start 0) end
  ...
  (flet ((get-end (vector)
    ...
    (let* ((end (or end (get-end byte-vector)))
           ...
           (out 0)
           (out-this-line 0))
      (declare (fixnum end n-bytes n-bytes-encoded in out out-this-line))
      (macrolet ((getbyte ()
...
(defun base64-decode-vector (base64-string &key (start 0) end
  ...
  (flet ((get-end (vector)
    ...
    (let* ((end (or end (get-end base64-string)))
           ...
           (result (make-array (ceiling (* 3 (the fixnum (- data-end start))) 4)     ;should be an integer anyway
           ...
           (putbyte-mode (cond ((subtypep decoded-byte-type 'character) :char)
                               ...
                               (t :coerce)))
           (in start))
      (declare (fixnum end data-end in))
...
(defun base64-encoded-size (decoded-size &key (max-line-length +base64-max-encoded-bytes-per-line+)
                                         (line-break-chars +base64-line-break-chars+))
  (declare (fixnum decoded-size))
  (let ((n-bytes-encoded (* 4 (ceiling (the fixnum (* 8 decoded-size)) 24))))
    (declare (fixnum n-bytes-encoded))
    (multiple-value-bind (lines chars)
        (floor n-bytes-encoded max-line-length)
      (declare (fixnum lines chars))
      (+ n-bytes-encoded (* (the fixnum (length line-break-chars))
                            (if (zerop chars)
                                (the fixnum  (1- lines))
                              lines))))))
...
(defun base64-decoded-size (encoded-size &key
  ...
  (declare (fixnum encoded-size pad-chars max-line-length))
  (assert (<= 0 pad-chars 2))
  (let ((encoded-data-size (multiple-value-bind (lines chars-remaining)
                               (floor encoded-size (+ max-line-length (the fixnum (length line-break-chars))))
                             (+ chars-remaining (the fixnum (* lines max-line-length))))))
    (declare (fixnum encoded-data-size))
    (unless (zerop (the fixnum (mod encoded-data-size 4)))
      ;; somethings wrong, assume the worst
      (return-from base64-decoded-size encoded-size))
    (ceiling (* 3 (the fixnum (- encoded-data-size (if (= pad-chars 2) 1 0)))) 4)))
