...
#-(version>= 6 0)
(defpackage "EXCL" (:export "SOCKET-ERROR"))
...
(defpackage "WWW-UTILS"
  ...
  (:import-from #:excl #:if* #:without-package-locks)
...
"Returns the amount of time the process has been up, in seconds."
  #+(and :allegro (version>= 7 0)) 0
  #+(and :allegro (version< 7 0)) (mp::process-start-secs process)
)
;;-------
...
#-(and :allegro :sparc :solaris2 (version>= 6 0))
...
;------------
; simple version needed in connection.lisp
...
;;;KAP this is no valid definition
#+no
(deftype end-of-chunk-transfer-decoding excl::socket-chunking-end-of-file)
;;;Kap this at least compiles, but is a condition, not a type
;;;Have to find the use of it
(deftype end-of-chunk-transfer-decoding () 'excl:socket-chunking-end-of-file)
...
(defun build-services-table ()
  ...
  (let ((fname #-mswindows (probe-file "/etc/services")
    ...
    (flet ((insert-data (name port protocol alt-names)
                        ;; data is (alt-name1 ... protocol port name)
                        ;; put in the database
                        #+no (format t "Entry ~a ~a ~a ~%" name port protocol)
                                  ...
                                  (loop (multiple-value-setq (aname pos)
                                        ...
                                        (if* aname 
                                           then (push aname alts)
                                                ;;;KAP alts may be empty
                                           else (progn 
                                                  (insert-data name port protocol nil)
                                                  (return nil)))
...
(defun tcp-service-port-number (protocol &optional error-p)
  (flet ((return-port (key)
                      (let ((ans (service-protocol-port key :tcp)))
                        (cond (ans (protocol&port-port  ans))
                              (error-p (error "Unknown protocol name, ~S" protocol))
                              (t nil))))) 
    (etypecase protocol
      (integer protocol)
      (keyword (return-port protocol))
      (symbol (return-port (intern (symbol-name protocol) (find-package :keyword))))
      (string (return-port (intern (string-upcase protocol) (find-package :keyword)))))))
...
(eval-when (load eval compile)
  (shadow 'common-lisp:stream-error :www-utils))

(define-condition stream-error (error) ())

(deftype stream-error () `(or excl:socket-error common-lisp:stream-error))
