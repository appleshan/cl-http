...
(defun patch-initialize-resourced-server (resource server set-stream set-host set-address)
  (initialize-resourced-server resource server set-stream set-host set-address))

#+before-70-182
(defun patch-deinitialize-resourced-server (server)
  (deinitialize-resourced-server 'http-server server))

(defun patch-deinitialize-resourced-server (resource server)
  (deinitialize-resourced-server resource server))
...
(defmacro with-stream-fd-watcher ((stream) . body)
   `(let ((#1=#:listen-socket-fd 
 	  #-(version>= 6 0) (slot-value ,stream 'excl::fn-in)
 	  #+(version>= 6 0) (slot-value ,stream 'excl::input-handle)
 	  ))
            ...
            (if stream
                (case (tcp-connection-state stream)
                   ...
                   (cond ((accept-connection-p)
                          (let* ((client-address (www-utils::foreign-host stream))
                            ...
                            (launch-process (process-namestring client-domain-name port)
                                            '(:priority 0)
                                            '%provide-service stream client-domain-name client-address timeout)
                  ...
                  (t #+FRANZ-INC
                     ...
                     :continue))
              (notify-log-window "Stream empty in %listen-for-connections. Port: ~a~%" )))
        (if non-stream
            (close non-stream)
          (notify-log-window "Non-Stream empty in %listen-for-connections. Port: ~a~%" port)))
      )
    )
  )
...
(defun ensure-http-protocol-on-port (port)
  ...
  (or (getf *acl-tcp-servers* port) (enable-http-stream-server port)))
...
;;;
(defun %provide-service (stream client-domain-name client-address timeout)
  (%provide-service-internal stream client-domain-name client-address timeout))
...
(define start (&key (log (get-line-argument "log=" 'symbol))
  ...
  #+ignore  ; now done in translations.lisp
