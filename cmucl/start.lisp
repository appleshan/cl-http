...
(defparameter *cl-http-server-version* '(70 218 0 4 4))
...
; get rid of the GC messages
#+cmu
(setq ext:*gc-verbose* nil)

; Less compiler messages.
...
(defvar *http-directory*
  ...
  (make-pathname :defaults (truename *load-pathname*)
		 :directory (butlast (pathname-directory (truename *load-pathname*)))
		 :name nil
		 :type nil)
...
(unless (find-package :mk)
  (let ((obj (make-pathname
	      :defaults (translate-logical-pathname "HTTP:cmucl;defsystem.lisp")
    ...
    (load (translate-logical-pathname "HTTP:cmucl;defsystem.lisp"))))
