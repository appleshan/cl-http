...
(defmethod stream-copy-until-eof (from-stream to-stream &optional (copy-mode :text))
  ...
  (ecase copy-mode
    ...
    ((:binary :crlf)
	 ...
	 (handler-case
	     (typecase from-stream
	       (WWW-UTILS::HTTP-STREAM
		(typecase to-stream
		  (WWW-UTILS::HTTP-STREAM
		   (LOOP FOR X OF-TYPE BASE-CHAR = (READ-CHAR FROM-STREAM)
		      DO (WRITE-CHAR X TO-STREAM)))
		  (t (LOOP FOR X OF-TYPE BASE-CHAR = (READ-CHAR FROM-STREAM)
			DO (WRITE-BYTE (CHAR-CODE X) TO-STREAM)))))
	       (t (typecase to-stream
		    (WWW-UTILS::HTTP-STREAM
		     (LOOP FOR X OF-TYPE (UNSIGNED-BYTE 8) = (READ-BYTE FROM-STREAM)
			DO (WRITE-CHAR (CODE-CHAR X) TO-STREAM)))
		    (t (LOOP FOR X OF-TYPE (UNSIGNED-BYTE 8) = (READ-BYTE FROM-STREAM)
			  DO (WRITE-BYTE X TO-STREAM))))))
	   (end-of-file () nil)))))))
