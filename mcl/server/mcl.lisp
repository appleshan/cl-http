...
;; It would be better to signal these conditions 5/16/2000 -- JCMa.
(deftype directory-not-found () 
  "Specialization of Common Lisp File-error in which the directory was not found on open."
  '(and condition file-error))

(export 'directory-not-found :www-utils) 
...
(define %parse-internet-address (ip-address)
 "Returns an IP-NUMBER which is integer denoting the address of host."
  ...
  (typecase ip-address
    (integer ip-address)
    (string (ccl::tcp-host-address ip-address))))
...
(defun %local-host-parsed-ip-number ()
   (handler-case
      #-ccl-5.0(ccl::%tcp-getaddr)
      #+ccl-5.0 (ccl::local-interface-ip-address)          ; use new definition-- JCMa 2/23/2003.
      (ccl:domain-error () 0)))
...
(define host-eq (host1 host2)
  ...
  (cond ((and (integerp host1) (integerp host2))
	...
	(t (eql (http::parse-internet-address host1)
...
(define file-stream-creation-date (file-stream)
  ...
  ;;(ccl::file-create-date file-stream)		;broken in MCL 5.0 8/26/2003 -- JCMa.
...
#+:CCL-3
(defmethod set-file-creation-date ((pathname pathname) (universal-time bignum) &optional error-p)
  (if error-p
    (ccl:set-file-create-date pathname universal-time)
    (ignore-errors (ccl:set-file-create-date pathname universal-time))))
...
(defmethod set-file-modification-date ((pathname pathname) (universal-time bignum) &optional error-p)
  (if error-p
    (ccl:set-file-write-date pathname universal-time)
    (ignore-errors (ccl:set-file-write-date pathname universal-time))))
...
(defun directory-list (pathname &rest options)
  ...
  (let ((pathnames (directory (merge-pathnames pathname "*.*")
    ...
    (loop with length and creation-date
          ...
          collect `(,path
                    ,.(when (pathname-directory-p path) (list :directory t)) 
  ...
  (let ((pathnames (directory (merge-pathnames pathname "*.*")
    ...
    (cond ((member :properties options)
	   (loop for path in pathnames
		 for directory-p = (pathname-directory-p path)
		 collect  (multiple-value-bind (length creation-date)
			      (file-properties path) 
			    `(,path 
			      ,.(cond (directory-p (list :directory t))
				      (length `(:length-in-bytes ,length))
				      (t nil))
			      ,.(when creation-date `(:creation-date ,creation-date))))))
	  (t pathnames))))
...
(export 'get-ramdisk-pathname :www-utils)
...
(define-macro %ensure-parsed-address (addr)
   `(etypecase ,addr
       (integer ,addr)
       (string (http::parse-internet-address ,addr))))
...
;;; STREAM HACKING
;;; 
(defmethod ccl::set-output-mode (stream mode)
   (ecase mode
      (:ascii (ccl::ascii-output-mode stream))
      (:binary (ccl::binary-output-mode stream))))

(defmethod ccl::set-input-mode (stream mode)
  (ecase mode
    (:ascii (ccl::ascii-input-mode stream))
    (:binary (ccl::binary-input-mode stream))))

(defmethod ccl::output-mode (s)
   (declare (ignore s))
   :ascii)

(defmethod ccl::input-mode (s)
   (declare (ignore s))
   :ascii)

(defmethod ccl::ascii-input-mode ((stream ccl::input-file-stream))
 t)

(defmethod ccl::binary-input-mode ((stream ccl::input-file-stream))
 nil)

(defmethod ccl::ascii-output-mode ((stream ccl::output-file-stream))
 t)

(defmethod ccl::ascii-output-mode ((stream ccl::output-stream))
 t)

(defmethod ccl::binary-output-mode ((stream ccl::output-file-stream))
 nil) 
...
direction can be :OUTPUT, :INPUT, or :BOTH."
   `(ccl::with-stream-mode-restored (,stream ,direction)
       ,(ecase direction
...
;; Added Suspended as an abortable whostate.  This state probably masks
;; some other condition and may be dangerous in some cases, but we're
;; going with it for the time being. -- JCMa 11/30/2000.
(defparameter *safe-server-abort-states* '("Suspended" "TCP In" "TCP Out" "TCP Finish" "TCP Closing" "TCP Accept")
...
;; Make the OpenTransport substrate respect http:*resolve-ip-addresses* -- JCMa 2/23/2003.
(add-initialization "Synchronize OpenTransport DNS Resolution" '(setq ccl::*use-resolver* http:*resolve-ip-addresses*))
...
;; server address and port might be. -- JCMa 2/26/2003.

#+OSX
(add-initialization "Add Loopback Virtual Host for MacOSX" '(add-loopback-host-virtual-host)
...
;; Very close to the MCL definition except it does not count wait time as
;; running like the MCL function. This allows us to find blocked processes.
;; We are assuming the New Scheduler here 12/19/2000 -- JCMa.
#+CCL-3
(defun process-idle-time (process)
  "Returns the amount of time since the process ran last, in sixtieths of a second."
  (ccl::%tick-difference (ccl::get-tick-count) (ccl::process-last-run-time process)))
...
(defmethod http::stream-encode-crlf-until-eof ((from-stream ccl::input-stream) (to-stream ccl::output-file-stream))
  (using-resource (line-buffer http::line-buffer http::*line-buffer-size*)
    (multiple-value-bind (writer to-sink)
                         (ccl:stream-writer to-stream)
      (loop with line and eof and delimiter and length
            ...
            do (unless (zerop length)
                 (with-fast-array-references ((array line vector))
                   delimiter                    ;ignore
                   (loop for idx upfrom 0 below length
                         for char = (char array idx)
                         do (funcall writer to-sink char)))
            (funcall writer to-sink #\Return)
            (funcall writer to-sink #\Linefeed))))))
...
#+ignore
(defmethod http::stream-encode-crlf-until-eof ((from-stream ccl::stream) (to-stream ccl::output-file-stream))
  (resources:using-resource (line-buffer http::line-buffer http::*line-buffer-size*)
    (ccl::with-fsopen-file (pb (ccl::stream-filename to-stream))
      (loop with line and eof and delimiter and length
            do (multiple-value-setq (line eof delimiter length)
            ...
            do (identity delimiter)        ; ignore
            unless (zerop length)
            do (vector-push-extend  #\Return line-buffer 2)
            (vector-push-extend  #\Linefeed line-buffer 1)
            (ccl::fswrite pb (+ 2 (the fixnum length)) line)
            until eof))))
...
(defmethod text-file-p ((pathname pathname)) 
   (let ((entry (get-file-type-pathname-extension-entry pathname)))
      (if entry
         (equalp (second entry) "TEXT")
         ;; try the portable method
         (call-next-method))))
