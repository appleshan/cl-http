...
;;;
;;; (c) Copyright  1995-96, 1999-2001, John C. Mallery
;;;     All Rights Reserved.
;;; 
;;;------------------------------------------------------------------- 
;;;
...
;;;
;;;   03/13/01  JCMa  Make HTTP::STREAM-COPY-BYTES¬perform CRLF decoding from HTTP stream when copy-mode is :text.
;;;   03/13/01  JCMa  Handle OPENTRANSPORT-END-OF-FILE orderly close in relevant functions:
;;;                   WWW-UTILS::%BUFFERED-STREAM-READ-DELIMITED-LINE, IO-BUFFER-COPY-UNTIL-EOF,
;;;                   IO-BUFFER-READ-ALL-BYTES-TO-FILE-STREAM, HTTP::ADVANCE-INPUT-BUFFER,
;;;                   HTTP::BINARY-STREAM-COPY-INTO-8-BIT-ARRAY, HTTP::CRLF-STREAM-COPY-INTO-STRING.
;;;   06/16/00  JCMa  Provide stream-reader and stream-writer methods
;;;   03/20/00  JCMa  Fixed io-buffer-copy-until-eof to work in chunking case.
;;;   12/17/99  JCMa  Revised chunked transfer decoding code. Fixed HTTP::ADVANCE-INPUT-BUFFER to increment the inptr.
...
(defmethod output-mode ((stream modal-ascii-or-binary-tcp-stream-mixin))
   (cond ((%tcp-stream-ascii-output-mode stream) :ascii)
            (t :binary)))

(defmethod set-output-mode ((stream modal-ascii-or-binary-tcp-stream-mixin) mode)
   (ecase mode
...
(defmacro with-stream-mode-restored ((stream direction) &body body)
   `(let (,.(when (member direction '(:output :both))
                  `((.output-mode. (ccl::output-mode ,stream))))
              ,.(when (member direction '(:input :both))
                     `((.input-mode. (ccl::input-mode ,stream)))))
       (unwind-protect
          (progn . ,body)
          ,(ecase direction
               (:output `(ccl::set-output-mode ,stream .output-mode.))
               (:input `(ccl::set-input-mode ,stream .input-mode.))
               (:both `(progn (ccl::set-output-mode ,stream .output-mode.)
                                     (ccl::set-input-mode ,stream .input-mode.)))))))

;;;------------------------------------------------------------------- 
;;;
;;; PROVIDE STREAM READER AND WRITER FUNCTIONS
;;;

;; Use stream methods so the the various handlers will in place -- JCMa 4/24/2001.
(defmethod stream-reader ((stream modal-ascii-or-binary-tcp-stream-mixin))
  (values (if (%tcp-stream-ascii-input-mode stream) #'stream-tyi  #'stream-read-byte) stream)) 

(defmethod stream-writer ((stream modal-ascii-or-binary-tcp-stream-mixin))
   (flet ((io-buffer-tyo (io-buffer char)
               (unless (typep io-buffer 'io-buffer)
	          (setq io-buffer (require-type io-buffer 'io-buffer)))
               (let ((char-code (char-code char)))
                  (prog1 (%io-buffer-write-byte io-buffer char-code)
                     (when (eql char-code #.(char-code #\Return))       ; symetrical CR-LF translation
                         (%io-buffer-write-byte io-buffer #.(char-code #\Linefeed))))))
	    (io-buffer-tyo-byte (io-buffer byte)
               (check-type byte integer)        ; don't accept random stuff, like characters -- JCMa 5/28/2001.
               (unless (typep io-buffer 'io-buffer)
	          (setq io-buffer (require-type io-buffer 'io-buffer)))
	       (%io-buffer-write-byte io-buffer byte)))
      (values (if (%tcp-stream-ascii-output-mode stream) #'io-buffer-tyo #'io-buffer-tyo-byte) (stream-io-buffer stream))))

;; Abstraction is too weak for modal streams because they can be in different modes for read and write. -- JCMa 6/4/2001.
(defmethod stream-element-type ((stream modal-ascii-or-binary-tcp-stream-mixin))
   (if (%tcp-stream-ascii-input-mode stream) 'base-character '(unsigned-byte 8)))
...
#|
(defmethod trace-output-chunk ((stream chunk-transfer-encoding-output-stream-mixin) start end)
   (trace-output-chunk (stream-io-buffer stream) start end))

(defmethod trace-output-chunk ((io-buffer io-buffer) start end)
...
(defstruct (chunked-transfer-decoder
              ...
              (:conc-name ctd-))
   (chunked-input nil :type (or boolean keyword))             ;whether output input-chunking is on or off
   (inside-input-chunk-p nil :type boolean)      ; whether we're processing a chunk
   (old-mode nil :type symbol)
   (input-content-length 0 :type integer)       ;total length of data
   (input-chunk-size 0 :type fixnum)    ;size of current chunk
   (input-scan-start 0 :type fixnum)    ;start of chunk
   (input-scan-end 0 :type fixnum)      ;end of chunk
   (input-scan-length 0 :type fixnum)   ;length of chunk data in current buffer
   (input-chunk-content-length 0 :type integer)         ; length of chunked data
   (input-chunk-crosses-buffer-p nil :type boolean)     ; chunk data crosses buffer boundary?
   (input-buffer-limit 0 :type integer)
   (input-chunks-received 0 :type fixnum)       ; number of chunks received
   (input-chunk-size-vector nil :type (or null vector))         ;vector to store chunk size
   (input-chunk-args-vector nil :type (or null vector)))        ;vector to store chunk args
...
#|(defun trace-input-chunk (io-buffer)
   ;;(declare (ignore start))
   (let* ((decoder (conn-decoder io-buffer))
           (input-buffer (io-buffer-inbuf io-buffer))
           (input-ptr (io-buffer-inptr io-buffer))
           (start (%ptr-difference input-ptr input-buffer))
           (end (+ start (ctd-input-scan-length decoder))))
      (loop for idx upfrom start below end
               for byte = (%get-byte input-buffer idx)
               for char = (code-char byte)
               do (write-char char *trace-output*))))|#
...
(define-condition end-of-chunk-transfer-decoding
   ...
   (:report (lambda (cond stream)
                   (format stream "HTTP Stream: End Chunked Data on ~S" (stream-error-stream cond))))
   (:documentation "Signalled when a complete HTTP resource has been successfully transferred.")) 

(defun %note-chunk-start (io-buffer &optional (decoder (conn-decoder io-buffer)))
  (multiple-value-bind (chunk-size chunk-args)
                       (%read-chunk-size io-buffer decoder)
    (declare (fixnum chunk-size)
             (ignore chunk-args))               ; don't know what to do with these yet.-- JCMa 8/28/1996.
    (with-chunk-transfer-decoding-traced
      (format *error-output* "~&Start-Chunk: ~D~&" (io-buffer-incount io-buffer)))
    (cond ((zerop chunk-size)
           (setf (ctd-inside-input-chunk-p decoder) nil
                 (ctd-chunked-input decoder) :end-of-chunk-transfer-decoding)
           #+ignore(break "End Chunked Transmission.")
           (signal 'end-of-chunk-transfer-decoding :stream (io-buffer-stream io-buffer)))
          (t ;; Handle fencepost case. moved from old decoding-advance -- JCMa 12/18/1999.
           (when (eql 0 (io-buffer-incount io-buffer))   ; How likely is this really?
             ( %io-buffer-advance io-buffer t t))
           ;; Set up chunk context
           (let* ((start (io-buffer-incount io-buffer))
             ...
             (setf (ctd-input-chunk-size decoder) chunk-size
                   ...
                   (io-buffer-incount io-buffer) length   ; set the end of buffer
                   (ctd-inside-input-chunk-p decoder) t)
             (incf (ctd-input-content-length decoder) chunk-size)
             (with-chunk-transfer-decoding-traced
               (format *error-output* "~&Start-Count: ~D~&" (io-buffer-incount io-buffer))))))) )

(declaim (inline %end-of-chunk-transfer-decoding-p))

(defun %end-of-chunk-transfer-decoding-p (decoder)
   (eql (ctd-chunked-input decoder) :end-of-chunk-transfer-decoding))
...
(defun %note-chunk-continue (io-buffer &optional (decoder (conn-decoder io-buffer)))
   (let* ((start (io-buffer-incount io-buffer))
             (chunk-size (ctd-input-chunk-size decoder))
             (chunk-content-length (ctd-input-chunk-content-length decoder))
             (end (max (- start (- chunk-size chunk-content-length)) 0))
             (length (- start end)))
      (declare (fixnum start chunk-size chunk-content-length end length))
      (setf (ctd-input-scan-start decoder) start
               (ctd-input-scan-end  decoder) end
               (ctd-input-scan-length  decoder) length
               (ctd-input-buffer-limit decoder) end  ; restore this count after chunk read
               (io-buffer-incount io-buffer) length)        ; set the end of buffer
      (incf (the fixnum (ctd-input-chunk-content-length decoder)) length)
      (setf (ctd-input-chunk-crosses-buffer-p decoder) (> chunk-size (ctd-input-chunk-content-length decoder)))))

(defun %note-chunk-end (io-buffer decoder)
   (with-chunk-transfer-decoding-traced
       (format *error-output* "~&End-Count: ~D~&" (io-buffer-incount io-buffer)))
   (setf (io-buffer-incount io-buffer) (ctd-input-scan-end decoder)  ; size of buffer restored
            (ctd-inside-input-chunk-p decoder) nil)
   (with-chunk-transfer-decoding-traced
       (format *error-output* "~&End-Chunk: ~D~&" (io-buffer-incount io-buffer)))
   (set-input-mode (io-buffer-stream io-buffer) :binary)
   (read-cr io-buffer)
   (read-lf io-buffer)
   (set-input-mode (io-buffer-stream io-buffer) (ctd-old-mode decoder)))

(defmethod chunk-transfer-decoding-mode ((s chunk-transfer-decoding-input-stream-mixin))
   (flet ((get-decoder (io-buffer coding-conn)
               (without-interrupts 
                 (or (conn-decoder io-buffer)
                       (let ((decoder (make-chunked-transfer-decoder)))
                          (setf (conn-decoder io-buffer) decoder
                                   (coding-conn-decoder coding-conn) decoder))))))
      (declare (inline get-decoder))
      (let* ((io-buffer (stream-io-buffer s))
                (coding-conn (coding-conn s))
                (decoder (get-decoder io-buffer coding-conn))) 
         (setf (coding-conn-saved-advance-function coding-conn) (io-buffer-advance-function io-buffer)
                  (coding-conn-saved-listen-function coding-conn) (io-buffer-listen-function io-buffer)
                  ; Always set correct advance and listen function for decoding mode.  -cvince 7/25/97
                  (io-buffer-advance-function io-buffer) #'decoding-advance
                  (io-buffer-listen-function io-buffer) #'decoding-listen
         ...
         (%note-chunk-start io-buffer decoder))))

(defmethod chunk-transfer-content-length  ((s chunk-transfer-decoding-input-stream-mixin) &aux io-buffer decoder)
   (cond ((and (setq io-buffer (stream-io-buffer s))
                     (setq decoder (conn-decoder io-buffer))
...
(defmethod chunk-transfer-decoding-mode-end ((s chunk-transfer-decoding-input-stream-mixin))
   (let* ((io-buffer (stream-io-buffer s))
             (coding-conn (coding-conn s))
             (decoder (conn-decoder io-buffer)))
      (setf (io-buffer-advance-function io-buffer) (coding-conn-saved-advance-function coding-conn)
               (io-buffer-listen-function io-buffer) (coding-conn-saved-listen-function coding-conn)
               (ctd-chunked-input decoder) nil)))

#|
;; old definition with some pretty kludgey junk from the PPC porter.
...
(defun decoding-advance (stream io-buffer readp errorp)
   (with-chunk-transfer-decoding-traced
       (format *error-output* "~&Next-Buffer-Start: ~D~&" (io-buffer-incount io-buffer)))
   (let* ((coding-conn (coding-conn stream))
             (decoder (coding-conn-decoder coding-conn))
             (saved-advance-function (coding-conn-saved-advance-function coding-conn)))
      (prog1
         (cond ((not (and (ctd-chunked-input decoder) (ctd-inside-input-chunk-p decoder)))
                    (with-chunk-transfer-decoding-traced
                        (format *error-output* "~&Next-Buffer: Standard ~D~&" (io-buffer-incount conn)))
                    (funcall saved-advance-function stream io-buffer readp errorp))
                  ;; if not first chunk, continue to resignal end of chunk after the end of chunked input has been reached.-- JCMa 5/13/1999.
                  ((%end-of-chunk-transfer-decoding-p decoder)
                    (and readp (signal 'end-of-chunk-transfer-decoding :stream (io-buffer-stream io-buffer))))
                  ((ctd-input-chunk-crosses-buffer-p decoder)
                    (prog1 (funcall saved-advance-function stream io-buffer readp errorp)
                       (unless (eql 0 (io-buffer-incount io-buffer))    ; no more data
                          (%note-chunk-continue io-buffer decoder))))
                  (t (%note-chunk-end io-buffer decoder)        ; end of chunk book keeping
                      (%note-chunk-start io-buffer decoder)     ; start of chunk book keeping
                      t))
         (with-chunk-transfer-decoding-traced
             (format *error-output* "~&Next-Buffer-End: ~D~&" (io-buffer-incount io-buffer)))))) 

(defun decoding-listen (stream io-buffer)
   (decoding-advance stream io-buffer nil nil))

(declaim (inline %chunked-input-mode-p))

(defun %chunked-input-mode-p (stream)
   (let (coding-conn decoder)
      (and (setq coding-conn (coding-conn stream)) 
...
; Symmetrical CR-LF translation 
(defmethod stream-tyi :around ((stream modal-ascii-or-binary-tcp-stream-mixin))
   (flet ((check-lf-char (stream)
               (let ((ch (call-next-method stream)))
                  (when (eql ch #\Linefeed)
                      (stream-untyi stream ch)))))
      (declare (inline check-lf-char))
      (let ((char (call-next-method stream)))
         (case char
            (#\Return (check-lf-char stream)))
         char))) 

;; handle end-of-chunk-transfer-decoding during chunked decoding.
(defmethod stream-tyi :around ((stream chunk-transfer-decoding-input-stream-mixin))
...
;; handle end-of-chunk-transfer-decoding during chunked decoding.
(defmethod stream-read-byte :around ((stream chunk-transfer-decoding-input-stream-mixin))
   (handler-case-if (%chunked-input-mode-p stream)
                             (call-next-method)
      (end-of-chunk-transfer-decoding () nil)))
...
;(defun io-buffer-read-all-bytes-to-file-stream (io-buffer stream)
...
(defun io-buffer-read-all-bytes-to-file-stream (io-buffer stream)
   (let* ((column.fblock (column.fblock stream))
      ...
      (with-io-buffer-locked (io-buffer)
          (handler-case-if 
             (chunked-input-mode-p (io-buffer-stream io-buffer))
             (loop while (if (eql 0 (io-buffer-incount io-buffer))
                                 (%io-buffer-advance io-buffer t nil)
                                 t)
                      for read-buffer =  (io-buffer-inptr io-buffer)
                      for buffer-size fixnum = (io-buffer-incount io-buffer)
                      do (fswrite pb buffer-size read-buffer)
...
(defmethod http::stream-copy-until-eof ((from-stream basic-tcp-stream) (to-stream output-file-stream) &optional (copy-mode :binary)
                                        &aux (conn (stream-io-buffer from-stream)))
  (ecase copy-mode
    ((:binary :crlf)
     (io-buffer-read-all-bytes-to-file-stream conn to-stream))
    (:text (http::stream-encode-crlf-until-eof from-stream to-stream))))
...
(defmethod http::stream-copy-until-eof ((from-stream modal-ascii-or-binary-tcp-stream) (to-stream modal-ascii-or-binary-tcp-stream) &optional copy-mode)
   (declare (ignore copy-mode))
   (io-buffer-copy-until-eof (stream-io-buffer from-stream) (stream-io-buffer to-stream)))

;; fast copying for other classes of buffered streams -- JCMa 4/24/2001.
(defmethod http::stream-copy-until-eof ((from-stream buffered-input-stream-mixin) (to-stream buffered-output-stream-mixin) &optional copy-mode)
   (declare (ignore copy-mode))
   (io-buffer-copy-until-eof (stream-io-buffer from-stream) (stream-io-buffer to-stream)))
...
(defmethod http::stream-copy-bytes ((from-stream modal-ascii-or-binary-tcp-stream) (to-stream output-file-stream) 
   ...
   (ecase copy-mode
      ((:binary :crlf)
        (if (< n-bytes 200)                  ; there is overhead involved in using the fast readers called in io-buffer-write-file
           (let ((io-buffer (stream-io-buffer from-stream)))
              (locally (declare (fixnum n-bytes))
                 (with-io-buffer-locked (io-buffer)
                     (multiple-value-bind (writer fblock)
                                                     (stream-writer from-stream)
                         (loop for count fixnum upfrom 0
                                  ...
                                  for byte = (%io-buffer-read-byte io-buffer t)
                                  do (funcall writer fblock byte))))))
           (%io-buffer-read-bytes-to-file-stream (stream-io-buffer from-stream) to-stream n-bytes)))
      (:text (http::stream-decode-crlf-bytes from-stream to-stream n-bytes))))
          ...
          (cond (delta
                     (locally (declare (fixnum delta))
                        (loop with remaining-bytes = delta
                                 ...
                                 for buffer-size fixnum = (io-buffer-incount io-buffer)
                                 for copy-n-bytes fixnum = (min remaining-bytes buffer-size)
                                 do (progn (%incf-ptr (io-buffer-inptr io-buffer) copy-n-bytes)
                                                (decf (io-buffer-incount io-buffer) copy-n-bytes)
                   ...
                   (t (loop while (if (eql 0 (io-buffer-incount io-buffer))
                                           ...
                                           t)
                               for buffer-size fixnum = (io-buffer-incount io-buffer)
                               do (progn (%incf-ptr (io-buffer-inptr io-buffer) buffer-size)
             ...
             (cond (n-bytes
                      ...
                      (t (cond ((null array)
                          ...
                          (handler-case-if  (%chunked-input-mode-p stream) 
                             ...
                             (end-of-chunk-transfer-decoding ()))       ;catch end of chunk transfer decoding
                          (setf (fill-pointer array) fill-pointer))))
         (values array fill-pointer))))
...
(defun www-utils::%buffered-stream-read-delimited-line (stream delimiters eof buffer &aux (index -1) error-p delimiter)
   ...
   (labels ((maybe-advance-buffer (io-buffer)
                    (if (eql 0 (io-buffer-incount io-buffer)) (%io-buffer-advance io-buffer t nil) t))
                (get-char (inptr)
