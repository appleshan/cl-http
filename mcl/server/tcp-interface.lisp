...
;;; Copyright John C. Mallery,  1995-1997, 1999-2000.
;;; All rights reserved.
...
(defun initialize-http-stream (resource http-stream host &optional (port *standard-http-port*) (timeout *server-timeout*)  (process ccl::*current-process*))
   (declare (ignore resource))
...
(defun deinitialize-http-stream (resource stream &aux state)
   (unwind-protect
      (progn (case (setq state (http-stream-connection-state stream))
                    ((:closed  #+Open-Transport :idle))
                    (:closing ;; Don't try closing streams that are already closing unless required. -- JCMa 4/24/2001.
                      (process-wait-with-timeout "TCP Await Close" ccl::*tcp-close-timeout* #'ccl::stream-closed-p stream)
                      (unless (ccl::stream-closed-p stream)
                         (notify-log-window  "While dellocating ~S, closing in abort mode TCP stream state, ~S,  on ~S."
                                                         resource (http-stream-connection-state stream) stream)
                         (close stream :abort t)))
                    ((:reset :syn-sent #-Open-Transport :syn-received)        ; safe states for closing in abort mode
                      (close stream :abort t))        ; close in abort mode
                    ((:listen :established)
                      (close stream))     ; orderly close
                    (t (notify-log-window  "While dellocating ~S, closing carefully due to unknown TCP stream state, ~S,  on ~S."
                                                       resource (http-stream-connection-state stream) stream)
                        (close stream)))
                 ;; Successfully closed the stream
                 (setq state :closed))
      ;; close in abort mode if we aborted form the code above, for example, due to user intervention-- JCMa 6/4/2001.
      (unless (eq state :closed)
         (notify-log-window  "Orderly close aborted while dellocating ~S, closing in abort mode TCP stream state, ~S,  on ~S."
                                         resource (http-stream-connection-state stream) stream)
         (close stream :abort t))
      ...
      (setf (ccl::stream-bytes-transmitted stream) 0
               ...
               (ccl::%tcp-stream-ascii-input-mode stream) t     ; return to ascii input mode -- JCMa 5/28/2001.
               (ccl::%tcp-stream-ascii-output-mode stream) t)   ; return to ascii output mode
      #+:CCL-4.3.1
      (setf (slot-value stream 'ccl::direction) :io)       ; broadcast streams blow out due to poor MCL stream modularity -- JCMa 3/2/2001.
      ;; deallocate process
      (shutdown-http-stream-process stream)
      (setf (ccl:tcp-stream-process stream) nil))
   ;; reset chunking data -- handled by unwind protects in chunking macros? -- JCMa 6/4/2001.
   stream)
...
(define http-service-enabled-p (&optional (ports (http-service-ports)))
...
keyword :ANY, this returns non-null when HTTP is enabled on some port."
   (flet ((http-service-on-port-p (port)
               (cdr (assoc port *server-control-alist*)))
            (any-active-http-service-p (ports)
               (loop for (port . streams) in *server-control-alist*
                        when (and streams port)
                        return ports
                        finally (return nil))))
      (declare (inline http-service-on-port-p))
      (etypecase ports
         ...
         (null (any-active-http-service-p ports))
   ...
   (flet ((log-dropped-connection (server)
            ...
            (handle-unhandled-error (server error)
               (handler-case
                  ...
                  (ccl:network-error () (close stream :abort t))
		  (condition (cond)
			     (ignore-errors (http::bug-report-error cond))))
      ...
      (resources:using-resource (server http-server stream client-host client-address)
         ...
         (let ((*server* server))
            (handler-case-if (not *debug-server*)
               ...
               (condition (cond) (handle-unhandled-error server cond))))))) 
...
(defvar *http-service-ports* nil   ; typically defaults to standard port, which is normally 80
  "The ports over which HTTP service is provided.")

(define http-service-ports ()
   "Returns the ports on which  HTTP service is allowed"
   (declare (values http-port-numbers))
   (or *http-service-ports*
         (setq *http-service-ports* (list *standard-http-port*))))

(export 'http-service-ports :http) 

(defun allow-http-service-for-port (port)
   (pushnew port *http-service-ports* :test #'=))

(defun disallow-http-service-on-port (port)
   "Removes HTTP service from PORT."
   (when (%listening-on-port-p port)
       (disable-http-service :on-ports port))
   (setq *http-service-ports* (delete port *http-service-ports*)))

(defun http-service-port-p (port)
   "Returns non-null POR is a known HTTP port."
   (member port (http-service-ports))) 

(declaim (inline ensure-http-protocol-on-port))
;; required by portable code
(define ensure-http-protocol-on-port (port)
   "Ensure that HTTP Service is provided on port."
   (allow-http-service-for-port port))

(define %enable-http-service (port &optional (listeners *number-of-listening-processes*))
   (cond ((http-service-port-p port)
              (unless (%listening-on-port-p port)
                 (allow-http-service-for-port port)
                 (dotimes (idx listeners)			;Provide multiple listening streams so that MACTCP is ready to accept the connection.
                    (%start-listening-for-connections port 0))))
            (t (error "The port, ~D, is not a known HTTP service port." port)))) 

(define enable-http-service (&key (on-ports (http-service-ports)) (listeners *number-of-listening-processes*))
   "Top-level method for starting up HTTP servers."
   (dolist (port (ensure-list on-ports))
      (ensure-http-protocol-on-port port)
      (%enable-http-service port listeners)
      ;; Advise the user.
      (notify-log-window "HTTP service enabled for: http://~A:~D/" (www-utils:local-host-domain-name) port))
   (expose-log-window)
   on-ports) 
...
ON-PORTS is a single port number, a list of port numbers,  or :ALL"
   (flet ((ensure-ports (port)
               (typecase port
                  (integer (list port))
                  (cons port)
                  (t (ecase port
                         (:all (%listening-on-ports)))))))
      (let ((ports (ensure-ports on-ports)))
         (dolist (port ports)
            (%stop-listening-for-connections port)
            ;; Advise the user. 
            (notify-log-window "HTTP service disabled for: http://~A:~D/"  (www-utils:local-host-domain-name) port))
         (expose-log-window)
         on-ports)))
