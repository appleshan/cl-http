...
9/17/00        Added functionality for selecting browsers.03/29/01       JCMa Added activate-p argument to browser-open-file
10/11/09       RJ Changed list of browsers, preferred browser.
...
(defparameter *preferred-browser* :Safari
  "One of the value of (LIST-ALL-BROWSERS)")

(defparameter *browsers*
  '((:Safari                      :|sfri|)    (:Camino                      :|MOZC|)    (:Firefox                     :|MOZB|)    (:Opera                       :|OPRA|)    (:Netscape-Navigator          :|MOSS|)
    (:Microsoft-Internet-Explorer :|MSIE|))
  "A list (browser-symbol browser-creator).")

(defun list-all-browsers ()
  "Returns a list of all known browsers."
  (mapcar #'first *browsers*))

(defun browser-creator (browser)
  "Returns the creator of a browser."
  (second (assoc browser *browsers*)))

; (browser-creator :Netscape-Navigator)

; (list-all-browsers)

(defun set-preferred-browser (browser)
  "Sets the preferred browser to the value of BROWSER."
  (assert (member browser *browsers* :key #'first))
  (setf *preferred-browser* browser))

; (set-preferred-browser :Microsoft-Internet-Explorer)
; (set-preferred-browser :Netscape-Navigator)
; (set-preferred-browser :foo)

(defun get-preferred-browser ()
  "Get the preferred browser."
  *preferred-browser*)

; (get-preferred-browser)

(defparameter *prefer-running-browser* t
  "Prefer a running browser, even though the requested browser
is a different one?")

(defun get-browser-creator ()
  "Get's the creator for the browser we want to use."
  (let ((preferred-browser-creator (browser-creator (get-preferred-browser))))
    (if *prefer-running-browser*
      (if (find-process-with-signature preferred-browser-creator)
        preferred-browser-creator
        (let ((running-browser (find-if (lambda (browser)
                                          (find-process-with-signature
                                           (browser-creator browser)))
                                        (list-all-browsers))))
          (if running-browser
            (browser-creator running-browser)
            preferred-browser-creator)))
      preferred-browser-creator)))
...
(defun open-url (url &key no-cache window
                     (browser (get-browser-creator))
                     activate-p)
  (let ((creator browser))
    (ensure-application-active creator)
    (when (find-process-with-signature creator)
      (with-aedescs (appleevent target reply)
        (create-signature-target target creator)
      ...
      (when activate-p (select-process creator)))))


;;; Get current URL from browser
(defun get-url (&key (browser (get-browser-creator)) (window 1))
  (let ((creator browser))
    (ensure-application-active creator)
    (when (find-process-with-signature creator)
      (let ((window-id (get-browser-window creator window)))
...
(defun get-browser-window (creator n)
  (when (find-process-with-signature creator)
    (with-aedescs (appleevent target reply list)
      (create-signature-target target creator)
...
(defun browser-open-file (file &key (window 1) anchor activate-p)
  (open-url (format nil "file:///~A~@[#~A~]"
                    (nsubstitute #\/ #\: (mac-namestring file))
                    anchor)
            :no-cache t
            :window (get-browser-window (get-browser-creator) window)
	    :activate-p activate-p))

(defun browser-file-name (&key (window 1) (browser (get-browser-creator)))
  (let ((creator browser))
    (let ((url (ignore-errors (get-url :browser creator :window window))))
      (when (stringp url)
