...
;;; Copyright 1994-2001, 2003, 2005-2006, John C. Mallery,  
;;; All rights reserved.
  ...
  (flet ((some-digit-char-p (string start end)
    ...
    (cond ((some-digit-char-p string start end)
           (let ((month-num (parse-integer string :start start :end end)))
             (if (< 0 month-num 13)
                 month-num
               (error 'http::time-parse-error :format-string "~A in ~S is a bad month." 
                      :format-args (list month-num (subseq string start end))))))
          (t (let ((month (subseq string start end)))
               (declare (dynamic-extent month))
               (cond ((gethash month *month-table*))
                     (t (error 'http::time-parse-error :format-string "~A is a bad month." 
...
(define month-string (month &optional (mode :long))
   ...
   (unless (and (integerp month) (< 0 month 13))
      (error 'http::time-parse-error :format-string "MONTH, ~s, is not an integer from 1 through 12"
             :format-args (list month)))
...
(define day-of-the-week-string (weekday &optional (mode :long))
  ...
  (unless (and (integerp weekday) (< -1 weekday 7))
    (error 'http::time-parse-error :format-string "WEEKDAY, ~S, is not an integer from 0 through 6."
           :format-args (list weekday)))
...
At 3am the next 3am is on the next day."
  (declare (fixnum offset) (integer reference-time))
  (multiple-value-bind (seconds minutes hours date month year day-of-the-week)
      (decode-universal-time reference-time)
    (declare (fixnum seconds minutes hours)
	     (ignore day-of-the-week))
    (+ (if (not (minusp (- (the fixnum (+ seconds (* 60 minutes) (* #.(* 60 60) hours)))
                           (the fixnum #.(* 3 60 60))	;3am
                           offset)))
	   #.(* 60. 60. 24.)			;plus 24 hours
	   0)
       offset					;offset
       (the integer (encode-universal-time 0 0 3. date month year (time-zone t)))))) ;time zone arg updates zone every evening
...
(defun string-translated-timezone (timezone &optional (start 0) (end (length timezone)))
  ...
  (flet ((hash-zone (string start end)
	   (when (alpha-char-p (aref string start))
      ...
      (cond ((hash-zone timezone start end))
	    ((= (- end start) 5)
	     (let* ((hours (parse-integer timezone :start (1+ start) :end (+ 3 start)))
		   ...
		   zone
		   (- zone))))
	    (t (error 'http::time-parse-error :format-string "~A is an invalid timezone"
                      :format-args (list timezone)))))))
...
(defun parse-iso-time (string &optional (start 0) (end (length string)))
  (multiple-value-bind (start end)
    ...
    (let* ((p1 (time-delimiter-position string  start  end))          ; year
           ...
           (p4 (and p3 (or (time-delimiter-position string  (1+ (the fixnum p3)) end))))      ;minutes
           (p5 (and p4 (or (time-delimiter-position string  (1+ (the fixnum p4))  end)))))     ; seconds
      (encode-universal-time (if p5 (parse-integer string :start (1+ (the fixnum p5)) :end end) 0)      ; second
                             (if p4 (parse-integer string :start (1+ (the fixnum p4)) :end (or p5 end)) 0)          ; minute
...
;; Seconds are optional according to RFC-822 -- JCMa 4/2/2000.
;; Time zones are optional for robustness -- JCMa 4/2/2000.
;; Weekday is optional. -- RJ 9.Oct.2003
(defun parse-rfc-850-and-822-time (string &optional (start 0) (end (length string)))
  (declare (fixnum start end))
  (flet ((get-timezone (start end)
	   (flet ((adjusted-time-zone-start (string start)
		    (declare (fixnum start))
		    (with-fast-array-references ((string string string))
		      (let ((char (aref string start)))
			(if (or (eql char #\+) (alpha-char-p char))
			    start
                          (1- start))))))
	     (declare (inline adjusted-time-zone-start))
	     (let* ((s1 (and start (time-delimiter-position-if-not string start end)))
		    (e1 (and s1 (time-delimiter-position string  (1+ (the fixnum s1)) end))))
	       (if s1
		   (string-translated-timezone string (adjusted-time-zone-start string s1) (or e1 end))
                 0)))))
    (declare (inline get-timezone))
    (multiple-value-bind (start end)
	(http::string-trim-bounds '(#\space) string start end)
      (let* ((weekday-p (alpha-char-p (aref string start)))
             (e1 (time-delimiter-position string  start  end))	; weekday ignored but assumed here
	     (s2 (time-delimiter-position-if-not string (if weekday-p e1 start) end))
	     ...
	     (seconds-p (and e6 (eql #\: (aref string e6))))
	     (s7 (and e6 (time-delimiter-position-if-not string e6 end)))
	     (e7 (and s7 (time-delimiter-position string  (1+ (the fixnum s7))  end))))	;  second
	(encode-universal-time (if seconds-p (parse-integer string :start s7 :end (or e7 end)) 0)	; second
			       ...
			       (get-timezone (if seconds-p e7 s7) end))))))	; time zone
...
(define iso-time-string-p (string &optional (start 0) (end (length string)))
   (declare (fixnum start)
            (ignore end))
   (and (digit-char-p (aref string start))
        (char= (aref string (+ start 4)) #\-)))
...
(defun parse-http-time-string (string &optional (start 0) (end (length string)))
  "This Returns a universal time when a string containing a time in RFC 850, 822
    or ANSI-C format."
  (flet ((handle-error (err) ;provides some additional information in error reports
           (cond (http::*debug-server* nil) ;when debugging, let errors  through for human attention
                 ((typep err 'http::time-parse-error) nil) ;don't resignal time errors
                 (t (error 'http::time-parse-error :format-string (report-string err))))))
    (handler-bind
        ((error #'handle-error))
      (cond ((ansi-c-time-string-p string start end)
...
;; Add error handling to deal with bad date headers 5/22/2001 -- JCMa.
(define parse-gmt-time (string &optional (start 0) (end (length string)))
  (declare (optimize (speed 3)))
  (flet ((report-parse-bug (err)
           (report-bug http:*server-mail-address*
                       "Time Parsing Error"
                       "Error in (www-utils:parse-gmt-time  ~S)~&Type: ~S~&Report: ~A"
                       (subseq string start end) (type-of err) (report-string err))))
    (cond ((and (not http:*debug-server*) (boundp 'http:*server*) http:*server*)
           (flet ((handle-error (err) ;provides some additional information in error reports
                    (when (typep err 'http::time-parse-error) ;condition defined afterwards
                      (when http::*report-time-parsing-errors* (report-parse-bug err))
                      (return-from parse-gmt-time (get-universal-time)))))
             (handler-bind
                 ((error #'handle-error))
               (parse-http-time-string string start end))))
          (t (parse-http-time-string string start end)))))
...
(defun default-pathname (&optional defaults host default-type default-version)
  (flet ((ensure-good-host (host)
           (etypecase host
             (null nil)
             (string (if (zerop (length host)) nil host))
             (symbol (if (eq host :unspecific) nil host)))))
    (let ((default-path http::*default-pathname*))
      (cond ((or defaults host default-type default-version)
             (let*  ((default-dir (pathname-directory default-path))
                     (good-host (or (ensure-good-host host) (ensure-good-host (pathname-host default-path))))
                     (path (make-pathname :host good-host
                                          :device (and (not good-host) (pathname-device default-path))
                                          :directory (and default-dir (cons :absolute (cdr default-dir)))
...
KEYWORDS is a list. It can contain thse keywords:
          ...
          :NORMAL -- Add to the initialization list.
          :NOW    -- Evaluate now and add to initialization list.
          :LAST   -- Add to the end of initialization list"
  ...
  (let ((entry (find name (symbol-value list-name) :test #'string-equal :key #'first)))
    ...
    (cond (entry
          ...
          ((member :last keywords)
           (setf (symbol-value list-name) `(,.(symbol-value list-name) (,name ,form nil ,@keywords))))
          (t (push `(,name ,form nil ,@keywords) (symbol-value list-name))))))
...
(define fixnum-microsecond-time ()
  "Returns the value of the low bits of the microsecond timer, as a fixnum.
Since you only see the low bits of the clock, the value can wrap around more quickly.
Genera returns 31 low bits."
  (declare (values low-bits top-1-bit))
...
;;;------------------------------------------------------------------- 
;;;
;;; LOCAL SCHEME definition for ports not supporting SSL
;;;
...
#-(or LispWorks scl ecl)
(declaim (ftype (function) http:http-service-enabled-p))

#+ecl
(declaim (ftype (function (t) t) http:http-service-enabled-p))
