...
;;;
;;; 04/23/01 jcma io-buffer-tyi added error-p second argument
;;; 02/28/01 jcma %io-buffer-read-bytes-to-file-stream written by Joswig added. 
;;; 03/20/00 jcma io-buffer-write-file don't error on zero-sized files.
...
(defun %io-buffer-read-byte (io-buffer errorp)
   ...
   (prog1
      ...
      ;; (incf (the fixnum ...)) does an overflow check
      (setf (io-buffer-bytes-read io-buffer) (the fixnum (1+ (the fixnum (io-buffer-bytes-read io-buffer)))))
...
(defun %io-buffer-tyi (io-buffer &optional error-p)
   (declare (optimize (speed 3) (safety 0)))
   (let ((byte (%io-buffer-read-byte io-buffer error-p)))
      (and byte (%code-char byte))))

(declaim (inline io-buffer-tyi))

(defun io-buffer-tyi (io-buffer &optional error-p dont-type-check)
...
(defmethod stream-tyi ((stream buffered-character-input-stream-mixin))
   (io-buffer-tyi (stream-io-buffer stream) nil t))
                      ...
                      (let ((outcount (io-buffer-outcount io-buffer)))
                         ...
                         (if (>= outcount bytes)
                            (progn
                               ...
                               (setf (io-buffer-outcount io-buffer) (the fixnum (- outcount bytes))
                                        (io-buffer-bytes-written io-buffer) (the fixnum (+ (the fixnum (io-buffer-bytes-written io-buffer)) bytes)))
                               (return))
                            (progn
                               (unless (eql 0 outcount)
                                  ...
                                  (setf (io-buffer-outcount io-buffer) 0
                                           (io-buffer-bytes-written io-buffer) (the fixnum (+ (the fixnum (io-buffer-bytes-written io-buffer)) outcount)))
                                  (decf bytes outcount)
                                  (incf start outcount))
                               (%io-buffer-force-output io-buffer nil))))))
                (if (simple-extended-string-p arr)
...
(defun io-buffer-write-file (io-buffer pathname &optional (start 0) end)
   (with-fsopen-file (pb pathname)
       (let* ((write-buf (io-buffer-outbuf io-buffer))
          ...
          (unless (eql 0 filesize)      ; don't error on Zero-sized files -- JCMa 12/17/1999
...
(defun %io-buffer-read-bytes-to-vector (io-buffer vector bytes start)
   ...
   (require-type bytes 'fixnum)
   (loop with fill-pointer fixnum = start
	    with bytes-remaining fixnum = bytes
	    until (eql 0 bytes-remaining)
	    while (if (eql 0 (io-buffer-incount io-buffer))
