...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;; Some menu stuff useful for CL-HTTP developers using Macintosh Common Lisp.

;;; Copyright: Rainer Joswig, 1996-2001, All rights reserved

;;; Author: Rainer Joswig, joswig@corporate-world.lisp.de, http://corporate-world.lisp.de/
;;; Developed with Macintosh Common Lisp 4.3
...
;;; CL-HTTP-MENU:*CL-HTTP-STARTUP-FILES*,
...
;;; 19/Apr/1997 - Changes to the ce dialog, uses present-to-string
...
;;; 17/Sep/2000 - Notice HTML Parser v10, and new CL-XML
;;;               New debug/trace options for W4 and Mailer.
;;;               No more ADD-SYSTEM, *CL-HTTP-SYSTEMS*.
;;;               Removed GET-CURRENT-BROWSER, SET-CURRENT-BROWSER.
;;;                 See http:mcl;server;control.lisp
;;;               Changed local browser preference functionality.
;;; 20/Sep/2000 - Edit System dialog is now modeless. RJ
;;; 02/Dec/2000 - Changed updating of Inspect menu. RJ
;;;               Fixed inspecting *PROXY-CACHE* lossage.
;;; 09/Dec/2000 - If (find-menu "CL-HTTP") is NIL, uses the value of *CL-HTTP-MENU*
;;;               Getting URLs from users, remove newlines - just in case...
;;; 15/Dec/2000 - Add Put-FILE menu-item. RJ
;;; 26/Dec/2000 - Add Get-FILE menu-item. RJ
...
(defun add-periods (string)
  (concatenate 'string string (string (code-char #+rmcl 8230 #-rmcl 201))))
...
(defun add-item-to-inspect (name form documentation &key update)
...
When UPDATE is T, the inspect menu will be updated."
  (check-type name string)
  (assert (or (null documentation) (stringp documentation))
          (documentation))
  (let ((item (list name form documentation))
    ...
    (if previous-item
      ...
      (setf *cl-http-inspect-description*
            (merge 'list (list item) *cl-http-inspect-description* #'string<
                   :key #'first))))
  (when update
    (update-inspect-menu)))
...
No versions means that all versions handle the capability.")
  ...
  (add-item-to-inspect "Standard Export Pathnames"
                       'http:*standard-export-pathnames*
  ...
  (add-item-to-inspect "CL-HTTP menu"
                       '*cl-http-menu* 
                       "The menu object for CL-HTTP - the Common Lisp Hypermedia Server"))
...
(defun make-menu-items-from-inspect-menu-description ()
  (loop for (name form documentation) in *cl-http-inspect-description*
        collect (make-instance 'inspect-item-menu-item
                  :menu-item-title name
                  :help-spec documentation
                  :form-to-inspect form)))

(defun update-inspect-menu ()
  (let ((menu (find-menu-item *cl-http-menu* "Inspect")))
    (mapc (lambda (item) (remove-menu-items menu item))
          (menu-items menu))
    (mapc (lambda (item) (add-menu-items menu item))
          (make-menu-items-from-inspect-menu-description))))
...
(defun make-menu-from-inspect-menu-description ()
  (when *cl-http-inspect-description*
    (make-instance 'menu
      :menu-title "Inspect"
      :menu-items (make-menu-items-from-inspect-menu-description))))
...
If a description with the same name exists, it will be replaced."
  ...
  (let ((item (list name function documentation))
    ...
    (if previous-item
      (setf (rest previous-item) (rest item))
      (setf *cl-http-tool-menu-description*
            (merge 'list (list item) *cl-http-tool-menu-description* #'string<
                   :key #'first))))
  (when activate
    (setup-cl-http-menu) ))
...
(defun make-menu-from-tool-menu-description ()
  (when *cl-http-tool-menu-description*
    (flet ((action (name)
             (lambda ()
               (let ((description (assoc name *cl-http-tool-menu-description*
                                         :test #'string-equal)))
                 (if description
...
(defun update-system-menu-for-system ()
  (labels ((find-cl-http-menu ()
             (or (find-menu "CL-HTTP") *cl-http-menu*))
           (update-menu (name items)
             (apply 'remove-menu-items (find-menu-item (find-cl-http-menu) name)
                    (menu-items (find-menu-item (find-cl-http-menu) name)))
             (apply 'add-menu-items (find-menu-item (find-cl-http-menu) name) items)))
...
;;; If a new system is being added to cl-user::*systems* it will be
    ...
    (make-instance 'menu-item
      ...
      :menu-item-action (lambda ()
                          (select-item-from-list
                           files
                           :window-title (format nil "Edit Files for ~a" name)
                           :selection-type :disjoint
                           :modeless t
                           :action-function (lambda (items) (map nil #'ed items))
    ...
    (if (and (fboundp 'compile-file)
             ...
             (or (not (probe-file compiled-file))
                 (let ((compiled-date (file-write-date compiled-file))
                   ...
                   (and compiled-date lisp-date (< compiled-date lisp-date))))
...
(defparameter *extension-files-description*
  (list (list "http:mcl;client;sysdcl.lisp" "Basic Client" 'cl-user::*cl-http-client*)
        ...
        (list "http:html-parser;v10;mac-sysdcl.lisp" "HTML Parser v10" 'cl-user::*html-parser*)
        ...
        (list "http:contrib;janderson;xml-2000-05-27;defsystem.lisp" "XML Parser")))
...
(http::define-preference-type
  ...
  :inferiors (:browser :prefer-running-browser)
...
(http::define-preference :browser
  ...
  :presentation-type `(www-present:member-sequence
                       ,(ccl:list-all-browsers))
  :prompt "Local Browser Name"
  :default-value-form (ccl:get-preferred-browser)
  :get-value-form (ccl:get-preferred-browser)
  :set-value-form (ccl:set-preferred-browser http::value)
  :description "The Browser you would like to use with your Mac.")

(pushnew :browser http::*remote-configuration-form-prototype*)

(http::define-preference :prefer-running-browser
  :name "Prefer Running Browser"
  :presentation-type `w3p:boolean
  :prompt "Prefer Running Browser"
  :default-value-form ccl:*prefer-running-browser*
  :get-value-form ccl:*prefer-running-browser*
  :set-value-form (setf ccl:*prefer-running-browser* http::value)
  :description "Prefer a running browser, even though the requested browser
is a different one?")

(pushnew :prefer-running-browser http::*remote-configuration-form-prototype*)

(defun select-and-set-browser ()
  "Select the preferred browser using a dialog."
  (let ((browser (first (select-item-from-list (ccl:list-all-browsers)
                                               :window-title "Select Browser"
                                               :selection-type :single))))
    (when browser
      (ccl:set-preferred-browser browser))))
    ...
    (cond ((and (consp presentation-type)
           ...
           (let* ((menu-items (loop for member in (second presentation-type)
                  ...
                  (menu (make-instance 'pop-up-menu
                          ...
                          :default-item (or (let ((pos (position
                    ...
                    (item (make-instance 'fred-dialog-item
                            ...
                            :view-position (add-points
                                            ...
                                            (make-point (+ (point-h (string-width (http::preference-name preference)
                                                           ...
                                                           10)
                                                        0))
                            ;; the following is correct, but doesn't work, since
                            ;; www-present:presentation-subtypep isn't really worked out. rj
                            :dialog-item-text #| (let ((value (funcall (http::preference-value-getter preference))))
                                                (if (www-present:presentation-subtypep
                                                     (www-present:presentation-type-of value)
                                                     presentation-type)
                                                  (www-present:present-to-string value presentation-type)
                                                  "")) |#
                            
                            (let ((value (funcall (http::preference-value-getter preference))))
                              (or (ignore-errors (www-present:present-to-string
...
(add-item-to-open-url "CL: Association of Lisp Users"
		      "http://www.lisp.org/"
		      "Online resource for Lisp users.")
...
(define-client-url-operation show-url-headers
                             (:title "Show URL Headers"
                                     :documentation "Displays the Headers for URL in the top Listener.
(If you press Option while clicking on the Show button, in a Fred window.) Asks for a full HTTP URL.")
...
(define-client-url-operation show-url 
                             (:title "Show URL"
                                     :documentation "Displays the data for URL in the top Listener.
(If you press Option while clicking on the Show button, in a Fred window.)  Asks for a full HTTP URL.")
...
(define-client-url-operation show-url-trace
                             (:title "Show URL Trace"
                                     :documentation "Display an HTTP 1.1 TRACE for URL in the top Listener.
(If you press Option while clicking on the Show button, in a Fred window.)  Asks for a full HTTP URL.")
...
(define-client-url-operation show-url-options
                             (:title "Show URL Options"
                                     :documentation "Display an HTTP 1.1 OPTIONS for URL in the top Listener.
(If you press Option while clicking on the Show button, in a Fred window.)  Asks for a full HTTP URL.")
...
; to do: Copy Mode is Text, determine the copy mode from the file's MIME type.

(defun put-file-action ()
  (let* ((file (choose-file-dialog))
         (name (pathname-name file))
         (type (pathname-type file))
         (default-url-string (default-url-string))
	 (pos (position #\/ default-url-string :test #'char= :from-end t)))
    (setf default-url-string
          (if pos
	      (concatenate 'string (subseq default-url-string 0 (1+ pos)) name "." type)
	      (concatenate 'string default-url-string "/" name "." type)))
    (let ((url-string (get-url-from-user "Put file to URL:"
                                         :initial-string default-url-string
                                         :window-title "Put File to URL")))
      (process-run-function "Put File to URL"
                            #'(lambda (file url-string)
				      (handler-case
					(http:copy-file file url-string :copy-mode :binary :report-stream *standard-output*)
					(error () (format *standard-output* "~&Copy from ~A to ~A failed." file url-string))))
                            file url-string))))

(defun make-put-file-item ()
  (make-instance 'menu-item
    :menu-item-title (add-periods "Put File to URL")
    :help-spec "Puts a file onto URL."
    :menu-item-action 'put-file-action))


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Get File

(defun get-file-action ()
  (process-run-function
   "Save URL data to File"
   (lambda ()
     (let* ((from (get-url-from-user "Get File from URL:"
                                     :initial-string (default-url-string)
                                     :window-title "Get URL"))
            (from-name-string (url::compute-object-name-string (url:url from)))
            (to (choose-new-file-dialog :name from-name-string)))
       (ccl::with-shown-progress (copy-task "get URL task")
         (ccl::format-progress copy-task "Getting URL ~a" from-name-string)
         (http:copy-file from to))))))


(defun make-get-file-item ()
  (make-instance 'menu-item
    :menu-item-title (add-periods "Get File from URL")
    :help-spec "Gets data from an URL and saves it into a file."
    :menu-item-action 'get-file-action))
...
log. For production servers you may want to turn it off.")
        (when (and (find-package "SMTP")
                   (find-symbol "*DEBUG-MAILER*" "SMTP")
                   (boundp (find-symbol "*DEBUG-MAILER*" "SMTP")))
          (make-toggle-var-item (find-symbol "*DEBUG-MAILER*" "SMTP")
                                "Debug Mailer"
                                "Debug support for the SMTP Mailer.
Not for production use."))
...
Not for production use."))
        (when (and (find-package "W4")
                   (find-symbol "*DEBUG-WALKER*" "W4")
                   (boundp (find-symbol "*DEBUG-WALKER*" "W4")))
          (make-toggle-var-item (find-symbol "*DEBUG-WALKER*" "W4")
                                "Debug W4 Web Walker"
                                "On error the walker enters a break loop.
Not for production use."))
        (when (and (find-package "W4")
                   (find-symbol "*TRACE-WALKER*" "W4")
                   (boundp (find-symbol "*TRACE-WALKER*" "W4")))
          (make-toggle-var-item (find-symbol "*TRACE-WALKER*" "W4")
                                "Trace the W4 Web Walker"
                                "Writes info to a report stream.
Not for production use."))
