...
;;;
;;; 05/15/01 jcma Abstract OT-CONN-TCP-ADDRESSES variants: OT-CONN-TCP-LOCAL-HOST, OT-CONN-TCP-LOCAL-PORT, 
;;;                           OT-CONN-TCP-REMOTE-ADDRESS, OT-CONN-TCP-REMOTE-HOST, OT-CONN-TCP-REMOTE-PORT
;;;                           and use them in STREAM-LOCAL-HOST, STREAM-LOCAL-PORT, STREAM-REMOTE-HOST, STREAM-REMOTE-PORT
;;; 04/24/01 jcma Fix ot-conn-buffer-advance so that error-p only controls signalling of eof
;;;                          All TCP errors are always signalled. 
;;; 03/13/01 kr & jcma Distinguish orderly from disorderly TCP close. Define new condition opentransport-end-of-file
;;;                and signal it instead of peer closed for orderly closes.
;;;                if there was an orderly disconnect.
;;; 02/28/01 jcma  define before method on STREAM-ABORT to close stream in abort mode
;;; 09/07/00 slh   more without-interrupts wrappers (from Kalman)
;;; 08/15/00 jcma ot-conn-buffer-advance signalling infrastructure added so that errorp argument is respected
...
;; dunno where this used to come from or why we need it etc etc but it isn't there now in new interfaces

(eval-when (:compile-toplevel :load-toplevel :execute)
  (defconstant TRAPS::$KTCPNAME "tcp"))
...
;;used only when orderly connection closed. Allows substrate to signal eof as appropriate.
(define-condition opentransport-end-of-file (opentransport-stream-closed end-of-file)
                           ((error-message :initform "Open Transport End of File")))
...
(defvar *opentransport-error-conditions*
  `((,#$kOTOutOfMemoryErr  opentransport-internal-memory-error)
    ...
    ((,#$kECONNRESETErr, #$ECONNRESET) opentransport-end-of-file :orderly)
...
(eval-when (:compile-toplevel :load-toplevel :execute)
(defconstant *opentransport-endpoint-states* #(:uninit :unbnd :idle :outcon :incon :dataxfer :outrel :inrel))
)
...
(defun ot-conn-buffer-advance (s conn readp errorp)
   (let* ((buf (io-buffer-inbuf conn)))
      ...
      (or (ot-conn-next-buffer conn)
            (when readp
                (setf (io-buffer-incount conn) -1)    ; flag to tell us if we have new buffer or not.
                ; Other methods would be faster and safer in aborts
                ; Wait until buffer available or stream closed.
                (process-poll-with-timeout "Net Input" (* 60 (the fixnum *tcp-read-timeout*))
                                                           #'(lambda (conn)
                                                                 (or (ot-conn-next-buffer conn)
                                                                       (pref (ot-conn-context conn) :ot-context.peerclosed))) conn)
                (when (eql -1 (io-buffer-incount conn))       ; new buffer actually received?
                    (setf (io-buffer-incount conn) 0)   ; restore the buffer count to something non-controversial
                    ;; Determine and signal the condition
                    (let ((eofp (ot-conn-eofp s conn))
                            context)
                       (unless (and eofp (null errorp)) ;; Always signal TCP errors. Don't signal OT EOF unless ERRORP non-null
                          (setq context (ot-conn-context conn))
                          (ot-error (if (pref context :ot-context.peerclosed)
                                            #$econnreset      ; closed by peer
                                            #$etimedout)      ; timeout
                                          (if eofp :orderly context)   ; distinguish orderly close
                                          conn))))))
      ; Return T if not EOF
...
(defun ot-conn-closed-p (conn)
   (let* ((endpoint (ot-conn-endpoint conn)))
      (or (null endpoint)
            (and (let ((endpoint-state (the fixnum (ot-%get-endpoint-state endpoint))))
                      (or
                        (<  endpoint-state #$T_DATAXFER)
                        (= endpoint-state #$T_INREL)))
...
(defclass opentransport-stream (buffered-character-io-stream-mixin buffered-binary-io-stream-mixin)
...
;; Close the stream when aborting so that no effort to push remaining data is
;; attempted 2/28/2001 -- JCMa.
(defmethod stream-abort :before ((s opentransport-stream))
...
(defun ot-conn-tcp-remote-address (conn)
   (let ((peer-address (ot-conn-peer-address conn)))
...
(defun ot-conn-tcp-addresses (conn)
   (let ((local-address (ot-conn-local-address conn))
           (peer-address (ot-conn-tcp-remote-address conn)))
...
(declaim (inline ot-conn-tcp-local-host ot-conn-tcp-local-port 
                        ot-conn-tcp-remote-host ot-conn-tcp-remote-port))

(defun ot-conn-tcp-local-host (conn)
   (cadr (ot-conn-local-address conn)))

(defun ot-conn-tcp-local-port (conn)
   (caddr (ot-conn-local-address conn))) 

(defun ot-conn-tcp-remote-host (conn)
   (cadr (ot-conn-tcp-remote-address conn))) 

(defun ot-conn-tcp-remote-port (conn)
   (caddr (ot-conn-tcp-remote-address conn)))

(defmethod stream-local-host ((s opentransport-tcp-stream))
   (let ((io-buffer (stream-io-buffer s nil)))
      (and io-buffer (ot-conn-tcp-local-host io-buffer))))

(defmethod stream-local-port ((s opentransport-tcp-stream))
   (let ((io-buffer (stream-io-buffer s nil)))
      (and io-buffer (ot-conn-tcp-local-port io-buffer))))

(defmethod stream-remote-host ((s opentransport-tcp-stream))
  (let ((io-buffer (stream-io-buffer s nil)))
    (and io-buffer (ot-conn-tcp-remote-host io-buffer))))

(defmethod stream-remote-port ((s opentransport-tcp-stream))
  (let ((io-buffer (stream-io-buffer s nil)))
    (and io-buffer (ot-conn-tcp-remote-port io-buffer)))) 
...
(defun opentransport-cleanup ()
  (do* ()
    ...
    (close (without-interrupts (pop *open-opentransport-streams*)))) ;;need without-interupts -- reti
  ; Close any freelisted endpoints.
  (do* ()
       ((null *opentransport-class-endpoint-freelists*))
    (let* ((cell ;;need without-interrupts -- reti
            (without-interrupts
             (pop *opentransport-class-endpoint-freelists*))))
      (dolist (ep (cdr cell))
        (#_OTCloseProvider ep))))
  (do* ()
  ...
  (do* ()
    ...
    (#_OTDestroyConfiguration (cdr (without-interrupts
                                    (pop *opentransport-config-alist*)))))
