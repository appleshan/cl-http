...
calls to PROCESS-YIELD. The standard CLIM-SYS:MAKE-PROCESS
...
or NIL."
  ...
  (let ((info (with-output-to-string (stream)
    ...
    (flet ((find-info (string)
	     (let ((loc (search string info #-CLISP :test #-CLISP #'char-equal)))
	       (when loc
		 ...
		 #+ACLPC (with-input-from-string (stream info :start loc)
			   (and (peek-char #\: stream nil nil)
				...
				(read stream nil nil)))))))
      (cond ((or (find-info #+CLISP "a variable declared GLOBAL" #-CLISP "global variable")
		 (find-info #+CLISP "a variable declared SPECIAL" #-CLISP "special variable"))
	     :special)
	    ((or (find-info #+CLISP ", a constant, " #-CLISP "constant")
...
(defmethod process-ordered-interrupts ((process t) &rest lambdas)
  (process-interrupt process #'(lambda ()
				 (loop for lambda in lambdas
				     do (process-yield #+ignore process)
					(funcall lambda)))))
