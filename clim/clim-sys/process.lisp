...
(defpackage "MINP"
  ...
  (:export "PROCESS-ACTIVE-P" "PROCESS-PRESET" "PROCESS-PRIORITY")
)
...
(defmethod process-priority ((process t))
  #+Symbolics
  (funcall #'process:process-priority process)
  #+(or Allegro LispWorks)
  (funcall #'mp:process-priority process)
  #+LispWorks
  (funcall #'mp::process-priority process)
  #+CCL-3
  (funcall #'ccl::process.priority process))

(defmethod (setf process-priority) (val (process t))
  #+Symbolics
  (setf (process:process-priority process) val)
  #+Allegro
  (setf (mp:process-priority process) val)
  #+LispWorks
  (mp:change-process-priority process val)
  #+CCL-3
  (setf (ccl::process.priority process) val))
...
(defmacro atomic-incf (reference &optional (delta 1))
  `(sys:atomic-incf ,reference ,delta))
 
#+(and LispWorks (not (or LispWorks4 LispWorks5)))
(defmacro atomic-decf (reference &optional (delta 1))
  `(sys:atomic-decf ,reference ,delta))
