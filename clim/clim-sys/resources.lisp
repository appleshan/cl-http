...
;;;
;;; (c) Copyright  1995, 1997, 1999, 2003, John C. Mallery
;;;     All Rights Reserved.
;;;
...
(defmacro with-atomic-execution (&rest body)
...
(defclass resource
    ...
    ((constructor :initform nil :initarg :constructor :reader resource-constructor)
     ...
     (clim-sys :initform nil :initarg :clim-sys :reader clim-sys-p)))
...
(defmethod %allocate-resource ((resource matching-resource) args &aux newly-created-p)
   (with-slots (free-list) resource
       (flet ((get-matching-object ()
          ...
          (let ((object (or (get-matching-object) (create-object)))
                  (initializer (resource-initializer resource)))
             (when initializer
	       (if (clim-sys-p resource)
		   (apply initializer object args)
...
(defmacro using-resource ((variable resource &rest args) &body body)
  ...
  `(let ((,variable (allocate-resource ',resource ,@args))) ;Resource now quoted to behave like CLIM-SYS resources - OBC
     (clim-sys:unwind-process			;MiniProc unwind-protect required - OBC
      (progn ,@body)
      (when ,variable
...
(defmethod fill-resource ((resource resource) &optional number-of-copies)
  #+ignore (declare (values n-copies))
  (with-slots (initial-copies parameters pool constructor description matcher free-list) resource
...
(defmethod initialize-resource ((resource resource) &rest args)
  (macrolet ((compiled-definition (fspec)
		  ...
		  (null nil)		;Okay to have no initializer
                  ...
                  (t (fdefinition ,fspec)))))
    (with-slots (constructor deinitializer initial-copies initializer name parameters pool clim-sys) resource
      (destructuring-bind (&key set-name set-parameters set-constructor
        ...
        (setq name set-name
	      ...
	      clim-sys from-clim-sys)
;;	(format t "~&Debug: resource clim-sys ~a" from-clim-sys)
        (unless pool
...
(defmethod initialize-resource #-CLISP :before #+CLISP :around ((resource matching-resource) &rest args)
  (macrolet ((compiled-definition (fspec)
               `(typecase ,fspec
                  (compiled-function ,fspec)
                  (t (fdefinition ,fspec)))))
