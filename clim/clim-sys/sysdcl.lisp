...
(let ((path (make-pathname :name "defsystem" :defaults *load-pathname*))
  ...
  (mapc #'(lambda (p)
	...
	;; changed to top of list attempt only -- 10/03
	(list (first
...
(defsystem "clim-sys"
  ...
  (:serial
   ...
   #+MINIPROC
   "minilock"
   "resources"	 ;JMCA resource with extensions to fit CLIM-SYS resources
   "resource"	 ;map resources definitions to CLIM-SY
   "util"
