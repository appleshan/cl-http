...
;;;
;;; (c) Copyright  1997, 2006, John C. Mallery
;;;     All Rights Reserved.
;;;
;;;------------------------------------------------------------------- 
;;;
  ...
  (flet ((write-heading (stream)
	   (with-anchor-noted (:reference *cl-http-home-page-url-string* :stream stream)
	     (image "/cl-http/icons/power.gif" "CL-HTTP" :stream stream
           ...
           (with-division (:alignment :center :inline-p t :stream stream)
             (fast-format stream "Web Lisp Listener")))
	 (write-blurb (stream)
	   (with-division (:alignment :center :stream stream)
	     (with-table (:cell-spacing 0 :cell-padding 10. :width .9 :stream stream)
...
functions include ARGLIST, APROPOS, APROPOS-LIST, DOCUMENTATION, and DESCRIBE."))
		     (with-paragraph (:stream stream)
...
through this interface, including via the value recording variables above.")))))))))
	...
	(with-document-body (:stream stream)
	  (with-section-heading (#'write-heading :stream stream)
	   ...
	   (with-division (:alignment :center :stream stream)
		(with-table
		  (:cell-spacing 0 :cell-padding 10. :width .9 :stream stream)
		  ;; Show type when applicable
		  (when (and typeout (not (null-string-p typeout)))
		    (with-table-row (:horizontal-alignment :center :vertical-alignment :middle :stream stream)
		      (with-table-cell (:column-span 3 :stream stream)
...
(pushnew :Web-Lisp-Listener *features*)
...
#+CL-HTTP-SSL
(export-url #u("/cl-http/listener.html" :port 8443 :protocol :https)
            :html-computed-form
            :form-function #'write-listener-form
            :expiration '(:no-expiration-header)
            :response-function #'read-eval-print-form
            :secure-subnets (list "127.0.0.1" (local-host-ip-address))
...
;(add-virtual-host-nick-name "127.0.0.1" 8443 (http::local-port-context 8443 :https))
;(add-virtual-host-nick-name "localhost" 8443 (http::local-port-context 8443 :https))
