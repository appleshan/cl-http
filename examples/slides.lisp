;;;   -*- Mode: LISP; Package: (slides :use (future-common-lisp html)); BASE: 10; Syntax: ANSI-Common-Lisp; Default-Character-Style: (:FIX :ROMAN :NORMAL);-*-

;;;
;;; (c) Copyright 1995, 2005, John C. Mallery
;;;     All Rights Reserved.
;;;
;;;------------------------------------------------------------------- 
;;;
...
;;; and display them via HTML 4.0  See the end of the file for an example that 
;;; exercises the features.

;;; Originally written in 1995 using mark up for Netscape 1.1N or clients
;;; supporting Netscape's extensions. Revised to use html 3.2 within the
;;; html 4.0 transitional DTD. 9/25/2005 -- JCMa

;; Update this example to use strict html 4.0 features and Cascading Style Sheets.

(eval-when (:load-toplevel :execute :compile-toplevel)
  (defpackage slides
    (:use future-common-lisp html)
...
(defun execute-operation (stream operation args)
  (ecase operation
    ...
    (:blink ;obscure tag in modern browsers
     (ns1.1:with-blinking (:stream stream)
       (%operation-dispatch stream args)))
    (:center
    ...
    (:table
     (destructuring-bind
         ...
         args
       (with-table (:border border :cell-spacing cell-spacing :cell-padding cell-padding :stream stream)
         (when caption
           (with-caption (:alignment caption-alignment :stream stream)
             (with-font (:size caption-size :stream stream)
               (etypecase caption
                 (string (write-string caption stream))
...
(defmethod initialize-element ((slide-show slide-show) init-args)
  (macrolet ((push-arg (arg)
	...
	(maybe-push-arg background-url)
        ...
        (push-arg slide-show) ;always present
    ...
    (when display-interval
      (let ((next (1+ number)))
        (cond ((< next (number-of-slides slide-show))
               (ns1.1:declare-refresh-rate display-interval
...
(defmethod write-body ((slide slide) stream)
  (with-slots (title icon line body) slide
