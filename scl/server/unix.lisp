...
(defmethod http::write-stack-backtrace ((error condition) stream &optional n-frames)
  (let ((*print-pretty* t))
    (debug:backtrace (or n-frames most-positive-fixnum) stream)))
...
(define ip-address-for-parsed-ip-address (ip-number)
  ...
  (ext:ip-address-string ip-number))
...
(defun local-protocol (http-stream)
  (let ((binary-stream (slot-value http-stream 'stream)))
    (if (typep binary-stream 'ssl:ssl-stream)
	:https
	:http)))

(defun local-port (http-stream)
  (let ((fd (sys:fd-stream-fd http-stream)))
    (multiple-value-bind (server-address server-port)
	(ext:get-socket-host-and-port fd)
      (declare (ignore server-address))
      server-port)))

(defun foreign-port (http-stream)
  (let ((fd (sys:fd-stream-fd http-stream)))
    (multiple-value-bind (client-address client-port)
	(ext:get-peer-host-and-port fd)
      (declare (ignore client-address))
      client-port)))
...
(defmethod file-length-in-bytes ((pathname pathname) &optional new-length)
  ...
  (unless (pathname-directory-p pathname)
    (multiple-value-bind (result dev-or-err inode mode nlink uid gid rdev size atime mtime)
       	 (unix:unix-stat (ext:unix-namestring pathname))
      (declare (ignore result dev-or-err inode mode nlink uid gid rdev atime mtime))
      size)))
...
(defmethod set-file-modification-date ((pathname pathname) (universal-time integer) &optional error-p)
  (let ((mtime (- universal-time lisp::unix-to-universal-time))
	(unix-filename (ext:unix-namestring pathname nil)))
    (multiple-value-bind (winp errno)
	(unix:unix-utimes unix-filename mtime 0 mtime 0)
      (when (and (not winp) error-p)
	(error "~@<Error setting modification date for ~S: ~S~@:>"
	       pathname (unix:get-unix-error-msg errno)))))
  universal-time)
...
(defmethod compress-file ((pathname pathname) mode &key delete)
  (ecase mode
    (:compress
     (let* ((name (pathname-name pathname))
	    (type (pathname-type pathname))
	    (target-name (if (stringp type)
			     (concatenate 'string name "." type)
			     name))
	    (target-pathname (make-pathname :name target-name
					    :type "gz"
					    :defaults pathname)))
       (with-open-file (input pathname :direction :input
			      :element-type '(unsigned-byte 8))
	 (with-open-file (output target-pathname :direction :output
				 :element-type '(unsigned-byte 8))
	   (let ((gzip-stream (ext:make-gzip-output-stream output)))
	     (loop
	      (let ((byte (read-byte input nil nil)))
		(unless byte
		  (return nil))
		(write-byte byte gzip-stream)))
	     (close gzip-stream))))
       (when delete
	 (delete-file pathname))
       (values target-pathname t)))
    (:decompress
     (let* ((source-name (pathname-name pathname))
	    (type (pathname-type pathname)))
       (assert (equalp type "gz"))
       (multiple-value-bind (name type version)
	   (lisp::extract-name-type-and-version source-name 0 (length source-name))
	 (declare (ignore version))
	 (let ((target (make-pathname :name name
				      :type type
				      :defaults pathname)))
	   (with-open-file (input pathname :direction :input
				  :element-type '(unsigned-byte 8))
	     (with-open-file (output target :direction :output
				     :element-type '(unsigned-byte 8))
	       (let ((gunzip-stream (ext:make-gunzip-input-stream input)))
		 (loop
		  (let ((byte (read-byte gunzip-stream nil nil)))
		    (unless byte
		      (return nil))
...
(define live-connection-p (http-stream)
  ...
  (let ((binary-stream (slot-value http-stream 'stream)))
    (and (open-stream-p binary-stream)
	 (handler-case
	     (ext:get-peer-host-and-port
	      (sys:fd-stream-fd binary-stream))
	   (error ())))))
...
(defun make-process (process-name &key &allow-other-keys)
...
(defun process-run-time (process)
  ...
  microseconds."
  (cond ((eq process (mp:current-process))
...
(define-macro atomic-conditional-replacef (reference predicate new-value)
  ...
  (let ((old-value (gensym))
    ...
    `(thread:with-exclusive-execution
...
This macro ensures atomic execution of BODY."
  (format t "~&~@<*** ~@;Warning: use of without-preemption is depreciated and is being ignored which could lead to data corruption.  The code may need to be rewritten to use locks for synchronization.~@:>~%")
  `(progn . ,body))
...
(defun cl-http-initialise (switch)
  (let ((init-file (or (ext:cmd-switch-value switch)
		       ...
		       "http:scl;examples;cl-http-init.lisp")))
...
(define copy-vector-portion (from-vector from-start from-end to-vector to-start to-end)
  ...
  (flet ((default-copy ()
    ...
    (typecase from-vector
      (string
       (kernel::with-array-data ((from-vector from-vector)
			     (from-start from-start)
			     (from-end from-end))
	 (typecase to-vector
	   (string
	    (kernel::with-array-data ((to-vector to-vector)
				  (to-start to-start)
				  (to-end to-end))
	      (loop for idx1 upfrom from-start below from-end
		    for idx2 upfrom to-start below to-end
